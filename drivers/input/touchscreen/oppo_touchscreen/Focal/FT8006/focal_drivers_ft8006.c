...
static int focal_enable_edge_limit(struct chip_data_ft8006 *chip_info, bool enable)
{
    ...
    if (enable) {
        ...
        ret |= touch_i2c_write_byte(chip_info->client, chip_info->reg_info.FTS_REG_EDGESCREEN_Y2, chip_info->grip_area.ver_edgescreen_y2/8);
        ret |= touch_i2c_write_byte(chip_info->client, chip_info->reg_info.FTS_REG_EDGESCREEN_DIR, chip_info->grip_area.edgescreen_switch-1);
    } else {
...
static int focal_get_chip_info(void *chip_data)
{
    ...
    reg_info->FTS_REG_EDGESCREEN_Y1 = 0x8D;
    reg_info->FTS_REG_EDGESCREEN_Y2 = 0x8E;
    reg_info->FTS_REG_EDGESCREEN_DIR = 0x8F;
...
static void focal_set_grip_handle(void *chip_data, int para_num, char *buf)
{
    uint32_t grip_bit = 0;
    char left_para[PAGESIZE] = {0};
    struct chip_data_ft8006 *chip_info = (struct chip_data_ft8006 *)chip_data;
...
struct focal_debug_func focal_debug_ops =
{
    ...
    .set_grip_handle                = focal_set_grip_handle,
};
...
static void focal_main_register_read(struct seq_file *s, void *chip_data)
{
    ...
    //edge screen
    seq_printf(s, "edge:dir(%d), y1(%d), y2(%d).\n",\
                    touch_i2c_read_byte(chip_info->client, chip_info->reg_info.FTS_REG_EDGESCREEN_DIR),\
                    touch_i2c_read_byte(chip_info->client, chip_info->reg_info.FTS_REG_EDGESCREEN_Y1),\
                    touch_i2c_read_byte(chip_info->client, chip_info->reg_info.FTS_REG_EDGESCREEN_Y2));
