...
static void goodix_enable_game_mode(struct chip_data_gt9286 *chip_info, bool enable);
...
static int goodix_enable_edge_limit(struct chip_data_gt9286 *chip_info, bool enable)
{
    ...
    if (enable) {
        ...
        if (edge_limit_last_status) {
            ret = goodix_send_cmd(chip_info, GTP_CMD_DISABLE_EDGE, 0);
        }
    }
    edge_limit_last_status = enable;
...
static int goodix_mode_switch(void *chip_data, work_mode mode, bool flag)
{
    ...
    switch(mode) {
        ...
        case MODE_GAME:
            goodix_enable_game_mode(chip_info, flag);
            break;
