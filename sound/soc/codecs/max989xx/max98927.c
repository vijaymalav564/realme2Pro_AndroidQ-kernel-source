...
#define SPECIAL_CODE (0xCACACACA)
...
#define CALIBRATE_FILE   "/persist/media/spkr_calibration.bin"
#define CALIBRATE_FILE_R   "/persist/media/spkr_calibration_r.bin"
...
struct dsm_info_t {
	...
	int32_t	num_channels;
 	int32_t maxTempQ19[2]; //Q19, in Hz, valid floating-point range = [0, 4194304)
	int32_t minTempQ19[2]; //Q19, in Hz, valid floating-point range = [0, 4194304)
	int32_t maxFcQ9[2]; //Q9, in Hz, valid floating-point range = [0, 4194304)
	int32_t minFcQ9[2]; //Q9, in Hz, valid floating-point range = [0, 4194304)
	int32_t maxRdcQ27[2]; //Q27, valid floating-point range = [0, 16)
	int32_t minRdcQ27[2]; //Q27
};
...
static int max98927_stream_mute(struct snd_soc_dai *codec_dai,
		int mute, int stream)
{
	struct snd_soc_codec *codec = codec_dai->codec;
	struct max98927_priv *max98927 = snd_soc_codec_get_drvdata(codec);
	...
	if (stream == SNDRV_PCM_STREAM_PLAYBACK) {
		...
		if (mute) {
			max98927_wrapper_write(max98927,
					MAX98927_Measurement_enables, 0x0);
			max98927->path_status = 0;
			pr_info("%s ------ disable max98927 capture\n", __func__);
		} else {
			#ifdef CONFIG_DEBUG_FS
			if (max98927->mono_stereo == 0x0
				|| (max98927->mono_stereo & 0x1)) {
				if (!rdc_check_valid(max98927->ref_RDC[MAX98927L])) {
			...
			if (max98927->mono_stereo & 0x2) {
				if (!rdc_check_valid(max98927->ref_RDC[MAX98927R])) {
					impedance = 0;
			...
			max98927_wrapper_write(max98927,
				MAX98927_Measurement_enables, 0x3);
		...
		struct snd_ctl_elem_value *ucontrol)
{
	...
	if (sel <= ((1 << MAX98927_AMP_VOL_WIDTH) - 1)) {
		...
		/* Jianfeng.Qiu@PSW.Multimedia.AudioDriver.Codec, 2017/05/19,
		 * Add for reduce vol for CT test.
		 */
		pr_info("%s: reduce vol because it is ct test build\n", __func__);
		sel = 34;
		#endif /* OPPO_AUDIO_CTTEST */
		...
		struct snd_ctl_elem_value *ucontrol)
{
	...
	if (i2c_states & MAX98927_CH0) {
		regmap_read(max98927->regmap[MAX98927L],
			MAX98927_PCM_to_speaker_monomix_A, &data);
		ucontrol->value.integer.value[0] =
