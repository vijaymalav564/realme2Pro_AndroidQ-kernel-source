...
** Version: 1.0
** Date : 2019/09/23
** Author: Kun.Zhou@ROM.Framework, 2019/09/23, add for hans freeze manager
**
** ------------------ Revision History:------------------------
** <author>      <data>      <version >       <desc>
** Kun Zhou    2019/09/23      1.0       build this module
****************************************************************/
 ...
 */
int hans_report(enum message_type type, int caller_pid, int target_uid, const char *rpc_name, int code)
{
	int len = 0;
	int ret = 0;
	...
	if (skb->len >= NLMSG_SPACE(0)) {
		...
		switch (data->type) {
			case LOOP_BACK:  /*Loop back message, only for native deamon and kernel handshake*/
				...
				hans_report(LOOP_BACK, -1, -1, "loop back", -1);
