...
void ohm_action_trig(int type)
{
        ...
        if (OHM_MEM_MON == type || OHM_SCHED_FSYNC == type || OHM_IOWAIT_HUNG == type) {
                if (!ohm_kobj) {
                        ohm_err("kobj NULL\n");
                        return;
                }
...
#define OHM_CTRL_IOWAIT_HUNG    BIT(OHM_IOWAIT_HUNG)


/*
ohm_ctrl_list    = 0x5a00ffff
ohm_logon_list = 0x5a00e005
ohm_trig_list    = 0x5a01e005
*/

/*Default*/
static int ohm_ctrl_list = OHM_LIST_MAGIC | OHM_CTRL_CPU_CUR | OHM_CTRL_MEMMON | OHM_CTRL_SCHEDTOTAL | OHM_CTRL_IOWAIT_HUNG;
...
bool ohm_iowait_hung_ctrl = true;
bool ohm_iowait_hung_logon = false;
bool ohm_iowait_hung_trig = false;
...
void ohm_trig_init(void)
{
	...
	ohm_iowait_hung_trig = (ohm_trig_list & OHM_CTRL_IOWAIT_HUNG) ? true : false;
	for (i = 0 ;i < OHM_SCHED_TOTAL ;i++ ) {
...
void ohm_logon_init(void)
{
	...
	ohm_iowait_hung_logon = (ohm_logon_list & OHM_CTRL_IOWAIT_HUNG) ? true : false;
	for (i = 0 ;i < OHM_SCHED_TOTAL ;i++ ) {
...
void ohm_ctrl_init(void)
{
	...
	ohm_iowait_hung_ctrl = (ohm_ctrl_list & OHM_CTRL_IOWAIT_HUNG) ? true : false;
	for (i = 0 ;i < OHM_SCHED_TOTAL ;i++ ) {
...
static ssize_t iowait_hung_read(struct file *filp, char __user *buff, size_t count, loff_t *off)
{
        ...
        len = sprintf(page, "iowait_hung_cnt: %u\n""iowait_panic_cnt: %u\n""iowait_hung_ctrl: %s\n",
                iowait_hung_cnt, iowait_panic_cnt, ohm_iowait_hung_ctrl? "true":"false");
