...
#define MASK_SIZE	32
#define MAX_IRQ_NO	1000

extern struct task_struct *oppo_get_cpu_task(int cpu);
...
struct oppo_irq_counter {
	unsigned int all_irqs_last;
	unsigned int all_irqs_delta;
	unsigned int irqs_last[MAX_IRQ_NO];
	unsigned int irqs_delta[MAX_IRQ_NO];
};

static struct oppo_irq_counter o_irq_counter;
static int irqno_sort[10];

static void updata_irq_counter(void)
{
	int n;
