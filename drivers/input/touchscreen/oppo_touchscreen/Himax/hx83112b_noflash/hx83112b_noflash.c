...
#include "hx83112b_noflash.h"

#define OPPO17001TRULY_TD4322_1080P_CMD_PANEL 29

/*******Part0:LOG TAG Declear********************/
#define TPD_DEVICE "himax,hx83112b_nf"
...
struct hx83112b_nf_report_data *hx83112b_nf_touch_data;
struct chip_data_hx83112b_nf *hx83112b_nf_chip_info;
int hx83112b_nf_touch_data_size = 128;
int HX83112B_NF_HW_RESET_ACTIVATE = 0;
static int HX83112B_NF_TOUCH_INFO_POINT_CNT   = 0;
int hx83112b_nf_lcd_vendor = 0;
int hx83112b_nf_irq_en_cnt = 0;


/*******Part0: SPI Interface***************/
#ifdef CONFIG_TOUCHPANEL_MTK_PLATFORM
const struct mtk_chip_config hx83112b_nf_spi_ctrdata = {
    .rx_mlsb = 1,
    .tx_mlsb = 1,
    .cs_pol = 0,
};
#else
const struct mt_chip_conf hx83112b_nf_spi_ctrdata = {
        .setuptime = 25,
        .holdtime = 25,
        .high_time = 3, /* 16.6MHz */
        .low_time = 3,
...
static ssize_t hx83112b_nf_spi_sync(struct touchpanel_data *ts, struct spi_message *message)
{
    int status;
...
static int hx83112b_nf_spi_read(uint8_t *command, uint8_t command_len, uint8_t *data, uint32_t length, uint8_t toRetry)
{
...
static int hx83112b_nf_spi_write(uint8_t *buf, uint32_t length)
{
    ...
    return hx83112b_nf_spi_sync(hx83112b_nf_pri_ts, &m);

}

static int hx83112b_nf_bus_read(uint8_t command, uint32_t length, uint8_t *data)
{
    int result = 0;
    uint8_t spi_format_buf[3];

    mutex_lock(&(hx83112b_nf_chip_info->spi_lock));
    spi_format_buf[0] = 0xF3;
    spi_format_buf[1] = command;
    spi_format_buf[2] = 0x00;

    result = hx83112b_nf_spi_read(&spi_format_buf[0], 3, data, length, 10);
    mutex_unlock(&(hx83112b_nf_chip_info->spi_lock));

    return result;
}

static int hx83112b_nf_bus_write(uint8_t command, uint32_t length, uint8_t *data)
{
    ...
    result = hx83112b_nf_spi_write(spi_format_buf, length + 2);
    mutex_unlock(&(hx83112b_nf_chip_info->spi_lock));
...
static int hx83112b_nf_power_control(void *chip_data, bool enable);
static int hx83112b_nf_get_chip_info(void *chip_data);
static int hx83112b_nf_mode_switch(void *chip_data, work_mode mode, bool flag);
static uint32_t hx83112b_nf_hw_check_CRC(uint8_t *start_addr, int reload_length);
static fw_check_state hx83112b_nf_hw_fw_check(void *chip_data, struct resolution_info *resolution_info, struct panel_info *panel_data);
static void hx83112b_nf_read_FW_ver(void);
static int hx83112b_nf_resetgpio_set(struct hw_resource *hw_res, bool on);

/*******Part2:Call Back Function implement*******/

//add for himax
void hx83112b_nf_flash_write_burst(uint8_t * reg_byte, uint8_t * write_data)
{
    uint8_t data_byte[8];
    int i = 0;
    int j = 0;
...
void hx83112b_nf_flash_write_burst_length(uint8_t *reg_byte, uint8_t *write_data, int length)
{
...
void hx83112b_nf_burst_enable(uint8_t auto_add_4_byte)
{
    uint8_t tmp_data[4];
    tmp_data[0] = 0x31;
...
void hx83112b_nf_register_read(uint8_t *read_addr, int read_length, uint8_t *read_data, bool hx83112b_nf_cfg_flag)
{
    uint8_t tmp_data[4];
    int ret;
...
void hx83112b_nf_register_write(uint8_t *write_addr, int write_length, uint8_t *write_data, bool hx83112b_nf_cfg_flag)
{
    uint8_t tmp_addr[4];
    uint8_t *tmp_data;

    int total_read_times = 0;
    uint8_t max_bus_size = MAX_I2C_TRANS_SZ,test=0;
    ...
    if (hx83112b_nf_cfg_flag == false) {
        ...
        TPD_DEBUG("%s, Entering - total write size=%d\n", __func__, total_size_temp);
        ...
        for (i = 0; i < (total_read_times); i++) {
            ...
            if (total_size_temp >= max_bus_size) {
                ...
                hx83112b_nf_flash_write_burst_length(tmp_addr, tmp_data, max_bus_size);
...
bool hx83112b_nf_sense_off(void)
{
    uint8_t cnt = 0;
    uint8_t tmp_addr[4];
    uint8_t tmp_data[4];
    ...
    do {
        ...
        if (tmp_data[0] == 0x0C) {
            ...
            hx83112b_nf_ic_reset(false, false);
            #endif
        }
    } while (cnt++ < 15);
...
bool hx83112b_nf_sense_off_mptest(void)
{
    uint8_t cnt = 0;
    uint8_t tmp_addr[4];
    uint8_t tmp_data[4];
...
bool hx83112b_nf_enter_safe_mode(void)
{
    uint8_t cnt = 0;
    uint8_t tmp_addr[4];
    uint8_t tmp_data[4];
...
void hx83112b_nf_interface_on(void)
{
    uint8_t tmp_data[5];
    uint8_t tmp_data2[2];
    int cnt = 0;
    ...
    do {
        ...
        // Check cmd
        hx83112b_nf_bus_read(0x13, 1, tmp_data);
        hx83112b_nf_bus_read(0x0D, 1, tmp_data2);
...
void hx83112b_nf_diag_register_set(uint8_t diag_command)
{
    uint8_t tmp_addr[4];
    uint8_t tmp_data[4];
...
bool hx83112b_nf_wait_wip(int Timing)
{
    uint8_t tmp_addr[4];
    uint8_t tmp_data[4];
    ...
    do {
        ...
        tmp_addr[1] = 0x00;
        tmp_addr[0] = 0x2C;
        hx83112b_nf_register_read(tmp_addr, 4, in_buffer, false);
...
void hx83112b_nf_sense_on(uint8_t FlashMode)
{
    uint8_t tmp_addr[4];
    uint8_t tmp_data[4];
    int retry = 0;
...
/**
 * hx83112b_nf_enable_interrupt -   Device interrupt ability control.
 * @chip_info: struct include i2c resource.
 * @enable: disable or enable control purpose.
 * Return  0: succeed, -1: failed.
 */
static int hx83112b_nf_enable_interrupt(struct chip_data_hx83112b_nf *chip_info, bool enable)
{
     TPD_DETAIL("%s enter, enable = %d.\n", __func__, enable);

    if (enable == true && hx83112b_nf_irq_en_cnt == 0) {
        enable_irq(chip_info->hx_irq);
        hx83112b_nf_irq_en_cnt = 1;
    } else if (enable == false && hx83112b_nf_irq_en_cnt == 1) {
        disable_irq_nosync(chip_info->hx_irq);
        hx83112b_nf_irq_en_cnt = 0;
    } else {
        TPD_DETAIL("irq is not pairing! enable= %d, cnt = %d\n", enable, hx83112b_nf_irq_en_cnt);
    }
...
#ifdef HX_ZERO_FLASH
struct hx83112b_nf_core_fp hx83112b_nf_core_fp;
struct hx83112b_nf_operation *hx83112b_nf_pzf_op = NULL;
//char *i_CTPM_firmware_name = "Himax_firmware.bin";
bool hx83112b_nf_auto_update_flag = false;
int HX83112B_NF_POWERONOF = 1;
...
void hx83112b_nf_mcu_sys_reset(void)
{
    hx83112b_nf_register_write (hx83112b_nf_pzf_op->addr_system_reset,  4,  hx83112b_nf_pzf_op->data_system_reset,  false);
}

int hx83112b_nf_dis_rload_0f(int disable)
{
    /*Diable Flash Reload*/
    int retry = 10;
    int check_val = 0;
    ...
    do {
        hx83112b_nf_flash_write_burst (hx83112b_nf_pzf_op->addr_dis_flash_reload,  hx83112b_nf_pzf_op->data_dis_flash_reload);
...
void hx83112b_nf_mcu_clean_sram_0f(uint8_t *addr, int write_len, int type)
{
    int total_read_times = 0;
    int max_bus_size = HX83112B_NF_MAX_TRANS_SZ;
    ...
    uint8_t tmp_addr[4];
    uint8_t tmp_data[HX83112B_NF_MAX_TRANS_SZ] = {0};
    ...
    if (total_size > HX83112B_NF_MAX_TRANS_SZ) {
        max_bus_size = HX83112B_NF_MAX_TRANS_SZ;
    }
    ...
    for (i = 0; i < HX83112B_NF_MAX_TRANS_SZ; i++) {
        tmp_data[i] = fix_data;
    }
    ...
    for (i = 0; i < (total_read_times); i++) {
        ...
        if (total_size_temp >= max_bus_size) {
            ...
            hx83112b_nf_flash_write_burst_length(tmp_addr, tmp_data,  total_size_temp % max_bus_size);
        }
        address = ((i+1) * max_bus_size);
...
void hx83112b_nf_mcu_write_sram_0f(const struct firmware *fw_entry, uint8_t *addr, int start_index, uint32_t write_len)
{
    int total_read_times = 0;
    int max_bus_size = HX83112B_NF_MAX_TRANS_SZ;
    int total_size_temp = 0;
    ...
    if (write_len > HX83112B_NF_MAX_TRANS_SZ) {
        max_bus_size = HX83112B_NF_MAX_TRANS_SZ;
    } else {
    ...
    */
    ...
    TPD_DETAIL("%s, ----END \n", __func__);
    //kfree (tmp_data);
    memset(hx83112b_nf_chip_info->tmp_data, 0, total_size);
}
int hx83112b_nf_parse_bin_cfg_data(const struct firmware *fw_entry, struct zf_info *zf_info_arr, uint8_t *FW_buf, uint8_t *sram_min)
{
    int part_num = 0;
    int i = 0;
    uint8_t buf[16];
...
static int hx83112b_nf_mcu_Calculate_CRC_with_AP(unsigned char *FW_content, int CRC_from_FW, int len)
{
    int i, j, length = 0;
    int fw_data;
    int fw_data_2;
...
static int hx83112b_nf_zf_part_info(const struct firmware *fw_entry)//zax
{
    int part_num = 0;
    int ret = 0;
    uint8_t buf[16];
    ...
    /*CFG*/
    cfg_sz = hx83112b_nf_parse_bin_cfg_data(fw_entry, zf_info_arr, FW_buf, sram_min);
    TPD_DETAIL("Now cfg_sz=%d\n", cfg_sz);
    if (cfg_sz < 0) {
        ret = LENGTH_FAIL;
        goto END_FUNC;
    } else {
        cfg_crc_sw = hx83112b_nf_mcu_Calculate_CRC_with_AP(FW_buf, 0, cfg_sz);
    ...
    /* 4. write to sram*/
    if (HX83112B_NF_POWERONOF == 1) {
        /* FW entity */
        retry = 0;
        do {
            hx83112b_nf_core_fp.fp_write_sram_0f(fw_entry, zf_info_arr[0].sram_addr, zf_info_arr[0].fw_addr, zf_info_arr[0].write_size);
            crc = hx83112b_nf_hw_check_CRC(zf_info_arr[0].sram_addr,  zf_info_arr[0].write_size);
            if (crc == 0) {
        ...
        hx83112b_nf_core_fp.fp_clean_sram_0f(zf_info_arr[0].sram_addr, zf_info_arr[0].write_size, 2);
    }
...
void hx83112b_nf_mcu_firmware_update_0f(const struct firmware *fw_entry)
{
    int retry = 0;
    int crc = -1;
    int ret = 0;
    ...
    bool reload = false;
    struct timespec timeStart1, timeEnd1,timeDelta1;
    getnstimeofday(&timeStart1);
    TPD_DETAIL("%s, Entering \n", __func__);
...
fw_reload:
    if (fw_entry == NULL || reload) {
        ...
        if(hx83112b_nf_chip_info->p_firmware_headfile->firmware_data) {
                request_fw_headfile->size = hx83112b_nf_chip_info->p_firmware_headfile->firmware_size;
                request_fw_headfile->data = hx83112b_nf_chip_info->p_firmware_headfile->firmware_data;
    ...
    /* first 48K */
        do {
            hx83112b_nf_core_fp.fp_write_sram_0f (tmp_fw_entry, hx83112b_nf_pzf_op->data_sram_start_addr, 0, HX_48K_SZ);
            crc = hx83112b_nf_hw_check_CRC (hx83112b_nf_pzf_op->data_sram_start_addr,  HX_48K_SZ);

            temp_addr[3] = 0x08;
            temp_addr[2] = 0x00;
            temp_addr[1] = 0xBF;
            temp_addr[0] = 0xFC;
            hx83112b_nf_register_read(temp_addr, 4, temp_data, false);
        ...
        /* if HX83112B_NF_POWERONOF!= 1, it will be clean mode! */
        /*config and setting */
        /*config info*/
        if (HX83112B_NF_POWERONOF == 1) {
            retry = 0;
            do {
                hx83112b_nf_core_fp.fp_write_sram_0f(tmp_fw_entry, hx83112b_nf_pzf_op->data_cfg_info, 0xC000, 128);//132
                crc = hx83112b_nf_hw_check_CRC(hx83112b_nf_pzf_op->data_cfg_info,  128);
                if (crc == 0) {
            ...
            hx83112b_nf_core_fp.fp_clean_sram_0f(hx83112b_nf_pzf_op->data_cfg_info, 128, 2);
        }
        /*FW config*/
        if (HX83112B_NF_POWERONOF == 1) {
            retry = 0;
            do {
                hx83112b_nf_core_fp.fp_write_sram_0f(tmp_fw_entry, hx83112b_nf_pzf_op->data_fw_cfg_p1, 0xC0FE, 528);//482
                crc = hx83112b_nf_hw_check_CRC(hx83112b_nf_pzf_op->data_fw_cfg_p1,  528);
                if (crc == 0) {
            ...
            hx83112b_nf_core_fp.fp_clean_sram_0f(hx83112b_nf_pzf_op->data_fw_cfg_p1, 528, 1);
        }
        if (HX83112B_NF_POWERONOF == 1) {
            retry = 0;
            do {
                hx83112b_nf_core_fp.fp_write_sram_0f(tmp_fw_entry, hx83112b_nf_pzf_op->data_fw_cfg_p2, 0xCA80, 48);
                crc = hx83112b_nf_hw_check_CRC(hx83112b_nf_pzf_op->data_fw_cfg_p2,  48);
                if (crc == 0) {
            ...
            hx83112b_nf_core_fp.fp_clean_sram_0f(hx83112b_nf_pzf_op->data_fw_cfg_p2, 48, 1);
        }
        if (HX83112B_NF_POWERONOF == 1) {
            retry = 0;
            do {
                hx83112b_nf_core_fp.fp_write_sram_0f(tmp_fw_entry, hx83112b_nf_pzf_op->data_fw_cfg_p3, 0xCA00, 128);
                crc = hx83112b_nf_hw_check_CRC(hx83112b_nf_pzf_op->data_fw_cfg_p3,  128);
                if (crc == 0) {
            ...
            hx83112b_nf_core_fp.fp_clean_sram_0f(hx83112b_nf_pzf_op->data_fw_cfg_p3, 128, 1);
        }
        ...
        if (HX83112B_NF_POWERONOF == 1) {
            retry = 0;
            do {
                hx83112b_nf_core_fp.fp_write_sram_0f(tmp_fw_entry, hx83112b_nf_pzf_op->data_adc_cfg_1, 0xD6B0, 1284);
                crc = hx83112b_nf_hw_check_CRC(hx83112b_nf_pzf_op->data_adc_cfg_1,  1284);
                if (crc == 0) {
            ...
            hx83112b_nf_core_fp.fp_clean_sram_0f(hx83112b_nf_pzf_op->data_adc_cfg_1, 1284, 2);
        }

        if (HX83112B_NF_POWERONOF == 1) {
            retry = 0;
            do {
                hx83112b_nf_core_fp.fp_write_sram_0f(tmp_fw_entry, hx83112b_nf_pzf_op->data_adc_cfg_2, 0xD358, 856);
                crc = hx83112b_nf_hw_check_CRC(hx83112b_nf_pzf_op->data_adc_cfg_2,  856);
                if (crc == 0) {
            ...
            hx83112b_nf_core_fp.fp_clean_sram_0f(hx83112b_nf_pzf_op->data_adc_cfg_2, 856, 2);
        }
        ...
        if (HX83112B_NF_POWERONOF == 1) {
            retry = 0;
            do {
                hx83112b_nf_core_fp.fp_write_sram_0f(tmp_fw_entry, hx83112b_nf_pzf_op->data_map_table, 0xE000, 1536);
                crc = hx83112b_nf_hw_check_CRC(hx83112b_nf_pzf_op->data_map_table,  1536);
                if (crc == 0) {
            ...
            hx83112b_nf_core_fp.fp_clean_sram_0f(hx83112b_nf_pzf_op->data_map_table, 1536, 2);
        }
    }
    /* switch mode*/
    if (HX83112B_NF_POWERONOF == 1) {
        hx83112b_nf_core_fp.fp_write_sram_0f(tmp_fw_entry, hx83112b_nf_pzf_op->data_mode_switch, 0xC30C, 4);
    } else {
        hx83112b_nf_core_fp.fp_clean_sram_0f(hx83112b_nf_pzf_op->data_mode_switch, 4, 2);
    }

    hx83112b_nf_hw_fw_check(hx83112b_nf_pri_ts->chip_data, &hx83112b_nf_pri_ts->resolution_info, &hx83112b_nf_pri_ts->panel_data);

    if(request_fw_headfile != NULL) {
        kfree(request_fw_headfile);
        request_fw_headfile = NULL;
    }
    getnstimeofday(&timeEnd1);
    timeDelta1.tv_nsec = (timeEnd1.tv_sec * 1000000000 + timeEnd1.tv_nsec) - (timeStart1.tv_sec * 1000000000 + timeStart1.tv_nsec);
    TPD_DETAIL("zax himax probe end time = %ld us\n", timeDelta1.tv_nsec / 1000);
    TPD_DETAIL("%s, END \n", __func__);
}
int hx83112b_nf_0f_op_file_dirly(char *file_name)
{
    int err = NO_ERR;
    ...
    hx83112b_nf_enable_interrupt(hx83112b_nf_chip_info, false);

    /* trigger reset */
    hx83112b_nf_resetgpio_set(hx83112b_nf_chip_info->hw_res, false); // reset gpio
    hx83112b_nf_resetgpio_set(hx83112b_nf_chip_info->hw_res, true); // reset gpio

    hx83112b_nf_core_fp.fp_firmware_update_0f (fw_entry);
    release_firmware (fw_entry);
...
int hx83112b_nf_mcu_0f_operation_dirly(void)
{
    ...
    hx83112b_nf_core_fp.fp_firmware_update_0f (hx83112b_nf_chip_info->g_fw_entry);
    //release_firmware (hx83112b_nf_chip_info->g_fw_entry);
...
int hx83112b_nf_mcu_0f_operation_test_dirly(char *fw_name)
{
    int err = NO_ERR;
    const struct firmware *fw_entry = NULL;
...
void hx83112b_nf_mcu_0f_operation(struct work_struct *work)
{
    int err = NO_ERR;
    //const struct firmware *fw_entry = NULL;
...
#ifdef HX_0F_DEBUG
void hx83112b_nf_mcu_read_sram_0f(const struct firmware *fw_entry, uint8_t *addr, int start_index, int read_len)
{
    int total_read_times = 0;
    int max_bus_size = HX83112B_NF_MAX_TRANS_SZ;
    int total_size_temp = 0;
    ...
    if (read_len > HX83112B_NF_MAX_RECVS_SZ) {
        max_bus_size = HX83112B_NF_MAX_RECVS_SZ;
    } else {
    ...
    for (i = 0; i < (total_read_times); i++) {
        if (total_size_temp >= max_bus_size) {
            hx83112b_nf_register_read(tmp_addr, max_bus_size, &temp_info_data[i*max_bus_size], false);
            total_size_temp = total_size_temp - max_bus_size;
        } else {
            hx83112b_nf_register_read(tmp_addr, total_size_temp % max_bus_size, &temp_info_data[i*max_bus_size], false);
        }
...
void hx83112b_nf_mcu_read_all_sram(uint8_t *addr, int read_len)
{
    int total_read_times = 0;
    int max_bus_size = HX83112B_NF_MAX_RECVS_SZ;
    int total_size_temp = 0;
    int total_size = 0;
    int address = 0;
    int i = 0;
...
void hx83112b_nf_mcu_firmware_read_0f(const struct firmware *fw_entry, int type)
{
    ...
    if (type == 0) { /* first 48K */
        ...
        hx83112b_nf_core_fp.fp_read_sram_0f (fw_entry, hx83112b_nf_pzf_op->data_cfg_info, 0xC000, 132);
        hx83112b_nf_core_fp.fp_read_sram_0f (fw_entry, hx83112b_nf_pzf_op->data_fw_cfg, 0xC0FE, 512);
        hx83112b_nf_core_fp.fp_read_sram_0f (fw_entry, hx83112b_nf_pzf_op->data_adc_cfg_1, 0xD000, 376);
        hx83112b_nf_core_fp.fp_read_sram_0f (fw_entry, hx83112b_nf_pzf_op->data_adc_cfg_2, 0xD178, 376);
...
int hx83112b_nf_0f_init(void)
{
    hx83112b_nf_pzf_op = kzalloc(sizeof(struct hx83112b_nf_operation), GFP_KERNEL);

    hx83112b_nf_core_fp.fp_reload_disable = hx83112b_nf_dis_rload_0f;
    hx83112b_nf_core_fp.fp_sys_reset = hx83112b_nf_mcu_sys_reset;
    hx83112b_nf_core_fp.fp_clean_sram_0f = hx83112b_nf_mcu_clean_sram_0f;
    hx83112b_nf_core_fp.fp_write_sram_0f = hx83112b_nf_mcu_write_sram_0f;
    hx83112b_nf_core_fp.fp_firmware_update_0f = hx83112b_nf_mcu_firmware_update_0f;
    hx83112b_nf_core_fp.fp_0f_operation = hx83112b_nf_mcu_0f_operation;
    hx83112b_nf_core_fp.fp_0f_operation_dirly = hx83112b_nf_mcu_0f_operation_dirly;
    hx83112b_nf_core_fp.fp_0f_op_file_dirly = hx83112b_nf_0f_op_file_dirly;
#ifdef HX_0F_DEBUG
    hx83112b_nf_core_fp.fp_read_sram_0f = hx83112b_nf_mcu_read_sram_0f;
    hx83112b_nf_core_fp.fp_read_all_sram = hx83112b_nf_mcu_read_all_sram;
    hx83112b_nf_core_fp.fp_firmware_read_0f = hx83112b_nf_mcu_firmware_read_0f;
    hx83112b_nf_core_fp.fp_0f_operation_check = hx83112b_nf_mcu_0f_operation_check;
#endif

    hx83112b_nf_in_parse_assign_cmd(hx83112b_nf_addr_dis_flash_reload, hx83112b_nf_pzf_op->addr_dis_flash_reload, sizeof(hx83112b_nf_pzf_op->addr_dis_flash_reload));
    hx83112b_nf_in_parse_assign_cmd(hx83112b_nf_data_dis_flash_reload, hx83112b_nf_pzf_op->data_dis_flash_reload, sizeof(hx83112b_nf_pzf_op->data_dis_flash_reload));
    hx83112b_nf_in_parse_assign_cmd(hx83112b_nf_addr_system_reset, hx83112b_nf_pzf_op->addr_system_reset, sizeof(hx83112b_nf_pzf_op->addr_system_reset));
    hx83112b_nf_in_parse_assign_cmd(hx83112b_nf_data_system_reset, hx83112b_nf_pzf_op->data_system_reset, sizeof(hx83112b_nf_pzf_op->data_system_reset));
    hx83112b_nf_in_parse_assign_cmd(hx83112b_nf_data_sram_start_addr, hx83112b_nf_pzf_op->data_sram_start_addr, sizeof(hx83112b_nf_pzf_op->data_sram_start_addr));
    hx83112b_nf_in_parse_assign_cmd(hx83112b_nf_data_sram_clean, hx83112b_nf_pzf_op->data_sram_clean, sizeof(hx83112b_nf_pzf_op->data_sram_clean));
    hx83112b_nf_in_parse_assign_cmd(hx83112b_nf_data_cfg_info, hx83112b_nf_pzf_op->data_cfg_info, sizeof(hx83112b_nf_pzf_op->data_cfg_info));
    hx83112b_nf_in_parse_assign_cmd(hx83112b_nf_data_fw_cfg_p1, hx83112b_nf_pzf_op->data_fw_cfg_p1, sizeof(hx83112b_nf_pzf_op->data_fw_cfg_p1));
    hx83112b_nf_in_parse_assign_cmd(hx83112b_nf_data_fw_cfg_p2, hx83112b_nf_pzf_op->data_fw_cfg_p2, sizeof(hx83112b_nf_pzf_op->data_fw_cfg_p2));
    hx83112b_nf_in_parse_assign_cmd(hx83112b_nf_data_fw_cfg_p3, hx83112b_nf_pzf_op->data_fw_cfg_p3, sizeof(hx83112b_nf_pzf_op->data_fw_cfg_p3));
    hx83112b_nf_in_parse_assign_cmd(hx83112b_nf_data_adc_cfg_1, hx83112b_nf_pzf_op->data_adc_cfg_1, sizeof(hx83112b_nf_pzf_op->data_adc_cfg_1));
    hx83112b_nf_in_parse_assign_cmd(hx83112b_nf_data_adc_cfg_2, hx83112b_nf_pzf_op->data_adc_cfg_2, sizeof(hx83112b_nf_pzf_op->data_adc_cfg_2));
    hx83112b_nf_in_parse_assign_cmd(hx83112b_nf_data_adc_cfg_3, hx83112b_nf_pzf_op->data_adc_cfg_3, sizeof(hx83112b_nf_pzf_op->data_adc_cfg_3));
    hx83112b_nf_in_parse_assign_cmd(hx83112b_nf_data_map_table, hx83112b_nf_pzf_op->data_map_table, sizeof(hx83112b_nf_pzf_op->data_map_table));
    hx83112b_nf_in_parse_assign_cmd(hx83112b_nf_data_mode_switch, hx83112b_nf_pzf_op->data_mode_switch, sizeof(hx83112b_nf_pzf_op->data_mode_switch));
...
bool hx83112b_nf_ic_package_check(void)
{
    uint8_t tmp_addr[4];
    uint8_t tmp_data[4];
    ...
    for (i = 0; i < 5; i++) {
        ...
        if(1)
        {
            HX83112B_NF_IC_TYPE = HX_83112A_SERIES_PWON;
            HX83112B_NF_IC_CHECKSUM = HX83112B_NF_TP_BIN_CHECKSUM_CRC;
            hx83112b_nf_0f_init();
            //Himax: Set FW and CFG Flash Address
            HX83112B_NF_FW_VER_MAJ_FLASH_ADDR   = 49157;  //0x00C005
            HX83112B_NF_FW_VER_MAJ_FLASH_LENG   = 1;
            HX83112B_NF_FW_VER_MIN_FLASH_ADDR   = 49158;  //0x00C006
            HX83112B_NF_FW_VER_MIN_FLASH_LENG   = 1;
            HX83112B_NF_CFG_VER_MAJ_FLASH_ADDR = 49408;  //0x00C100
            HX83112B_NF_CFG_VER_MAJ_FLASH_LENG = 1;
            HX83112B_NF_CFG_VER_MIN_FLASH_ADDR = 49409;  //0x00C101
            HX83112B_NF_CFG_VER_MIN_FLASH_LENG = 1;
            HX83112B_NF_CID_VER_MAJ_FLASH_ADDR = 49154;  //0x00C002
            HX83112B_NF_CID_VER_MAJ_FLASH_LENG = 1;
            HX83112B_NF_CID_VER_MIN_FLASH_ADDR = 49155;  //0x00C003
            HX83112B_NF_CID_VER_MIN_FLASH_LENG = 1;
            //PANEL_VERSION_ADDR = 49156;  //0x00C004
            //PANEL_VERSION_LENG = 1;
#ifdef HX_AUTO_UPDATE_FW
            g_i_FW_VER = i_CTPM_FW[HX83112B_NF_FW_VER_MAJ_FLASH_ADDR]<<8 |i_CTPM_FW[HX83112B_NF_FW_VER_MIN_FLASH_ADDR];
            g_i_CFG_VER = i_CTPM_FW[HX83112B_NF_CFG_VER_MAJ_FLASH_ADDR]<<8 |i_CTPM_FW[HX83112B_NF_CFG_VER_MIN_FLASH_ADDR];
...
void hx83112b_nf_power_on_init(void)
{
    uint8_t tmp_addr[4];
    uint8_t tmp_data[4];
...
void hx83112b_nf_read_OPPO_FW_ver(void)
{
    uint8_t cmd[4];
    uint8_t data[4];
...
uint32_t hx83112b_nf_hw_check_CRC(uint8_t *start_addr, int reload_length)
{
    uint32_t result = 0;
    uint8_t tmp_addr[4];
    uint8_t tmp_data[4];
    ...
    hx83112b_nf_flash_write_burst(tmp_addr, start_addr);
...
bool hx83112b_nf_calculateChecksum(bool change_iref)
{
    ...
    CRC_result = hx83112b_nf_hw_check_CRC(tmp_data, FW_SIZE_64k);
...
int hx83112b_nf_cal_data_len(int raw_cnt_rmd, int HX_MAX_PT, int raw_cnt_max)
{
    int RawDataLen;
    if (raw_cnt_rmd != 0x00) {
...
int hx83112b_nf_report_data_init(int max_touch_point, int tx_num, int rx_num)
{
    if (hx83112b_nf_touch_data->hx_coord_buf!=NULL) {
        kfree(hx83112b_nf_touch_data->hx_coord_buf);
    }

    if(hx83112b_nf_touch_data->diag_mutual != NULL) {
        kfree(hx83112b_nf_touch_data->diag_mutual);
    }

//#if defined(HX_SMART_WAKEUP)
    hx83112b_nf_touch_data->event_size = 128;
//#endif*/

    hx83112b_nf_touch_data->touch_all_size = 128; //himax_get_touch_data_size();

    HX83112B_NF_TOUCH_INFO_POINT_CNT = max_touch_point * 4 ;

    if ((max_touch_point % 4) == 0)
        HX83112B_NF_TOUCH_INFO_POINT_CNT += (max_touch_point / 4) * 4 ;
    else
        HX83112B_NF_TOUCH_INFO_POINT_CNT += ((max_touch_point / 4) +1) * 4 ;

    hx83112b_nf_touch_data->raw_cnt_max = max_touch_point/4;
    hx83112b_nf_touch_data->raw_cnt_rmd = max_touch_point%4;

    if (hx83112b_nf_touch_data->raw_cnt_rmd != 0x00) {//more than 4 fingers
        hx83112b_nf_touch_data->rawdata_size = hx83112b_nf_cal_data_len(hx83112b_nf_touch_data->raw_cnt_rmd, max_touch_point, hx83112b_nf_touch_data->raw_cnt_max);
        hx83112b_nf_touch_data->touch_info_size = (max_touch_point+hx83112b_nf_touch_data->raw_cnt_max+2)*4;
    } else {//less than 4 fingers
        hx83112b_nf_touch_data->rawdata_size = hx83112b_nf_cal_data_len(hx83112b_nf_touch_data->raw_cnt_rmd, max_touch_point, hx83112b_nf_touch_data->raw_cnt_max);
        hx83112b_nf_touch_data->touch_info_size = (max_touch_point+hx83112b_nf_touch_data->raw_cnt_max+1)*4;
    }

    if ((tx_num * rx_num + tx_num + rx_num) % hx83112b_nf_touch_data->rawdata_size == 0) {
        hx83112b_nf_touch_data->rawdata_frame_size = (tx_num * rx_num + tx_num + rx_num) / hx83112b_nf_touch_data->rawdata_size;
    } else {
    ...
    hx83112b_nf_touch_data->diag_mutual = kzalloc(tx_num * rx_num* sizeof(int32_t), GFP_KERNEL);
    if (hx83112b_nf_touch_data->diag_mutual == NULL) {
        goto mem_alloc_fail;
    }

    //#ifdef HX_TP_PROC_DIAG
    hx83112b_nf_touch_data->hx_rawdata_buf = kzalloc(sizeof(uint8_t)*(hx83112b_nf_touch_data->touch_all_size - hx83112b_nf_touch_data->touch_info_size),GFP_KERNEL);
    if (hx83112b_nf_touch_data->hx_rawdata_buf == NULL) {
        goto mem_alloc_fail;
    }
    //#endif

//#if defined(HX_SMART_WAKEUP)
    hx83112b_nf_touch_data->hx_event_buf = kzalloc(sizeof(uint8_t)*(hx83112b_nf_touch_data->event_size),GFP_KERNEL);
    if (hx83112b_nf_touch_data->hx_event_buf == NULL) {
        goto mem_alloc_fail;
    }
//#endif
...
//#if defined(HX_TP_PROC_DIAG)
    kfree(hx83112b_nf_touch_data->hx_rawdata_buf);
//#endif
//#if defined(HX_SMART_WAKEUP)
...
bool hx83112b_nf_read_event_stack(uint8_t *buf, uint8_t length)
{
    uint8_t cmd[4];
    ...
    hx83112b_nf_bus_read(0x30, length, buf);
...
int hx83112b_nf_zero_event_count = 0;
int hx83112b_nf_ic_esd_recovery(int hx_esd_event, int hx_zero_event, int length)
{
...
static int hx83112b_nf_resetgpio_set(struct hw_resource *hw_res, bool on)
{
    ...
    if (gpio_is_valid(hw_res->reset_gpio)) {
        ...
        msleep(HX83112B_NF_RESET_TO_NORMAL_TIME);
...
void hx83112b_nf_esd_hw_reset(struct chip_data_hx83112b_nf * chip_info)
{
    int ret = 0;
    int load_fw_times = 10;

    TPD_DETAIL("START_Himax TP: ESD - Reset\n");
    HX83112B_NF_ESD_RESET_ACTIVATE = 1;

    hx83112b_nf_enable_interrupt(hx83112b_nf_chip_info, false);

    do {
        hx83112b_nf_resetgpio_set(chip_info->hw_res, true); // reset gpio
        hx83112b_nf_resetgpio_set(chip_info->hw_res, false); // reset gpio
        hx83112b_nf_resetgpio_set(chip_info->hw_res, true); // reset gpio
    ...
    himax_report_all_leave_event(hx83112b_nf_pri_ts);*/

    hx83112b_nf_enable_interrupt(hx83112b_nf_chip_info, true);
}


int hx83112b_nf_checksum_cal(struct chip_data_hx83112b_nf * chip_info, uint8_t *buf,int ts_status)
{
...
//#endif
    ...
    /* Normal */
    if(ts_status == HX83112B_NF_REPORT_COORD) {
        length = hx83112b_nf_touch_data->touch_info_size;
    }

    /* SMWP */
    else if(ts_status == HX83112B_NF_REPORT_SMWP_EVENT) {
...
//#ifdef HX_ESD_RECOVERY
        if ((hx_esd_event == length || hx_zero_event == length)
           && (HX83112B_NF_HW_RESET_ACTIVATE == 0) && (HX83112B_NF_ESD_RESET_ACTIVATE == 0)) {
            shaking_ret = hx83112b_nf_ic_esd_recovery(hx_esd_event,hx_zero_event,length);
...
void hx83112b_nf_log_touch_data(uint8_t *buf,struct hx83112b_nf_report_data *hx83112b_nf_touch_data)
{
    int loop_i = 0;
    int print_size = 0;

    if (!hx83112b_nf_touch_data->diag_cmd) {
        print_size = hx83112b_nf_touch_data->touch_info_size;
    } else {
        print_size = hx83112b_nf_touch_data->touch_all_size;
    }
...
void hx83112b_nf_idle_mode(int disable)
{
    int retry = 20;
    uint8_t tmp_addr[4];
    uint8_t tmp_data[4];
...
void hx83112b_nf_reload_disable(int on)
{
    uint8_t tmp_addr[4];
    uint8_t tmp_data[4];
...
int hx83112b_nf_get_rawdata(struct chip_data_hx83112b_nf *chip_info, uint32_t *RAW, uint32_t datalen)
{
    ...
    uint8_t max_i2c_size = HX83112B_NF_MAX_RECVS_SZ;
    int address = 0;
    int total_read_times = 0;
    ...
    while (retry < 200) {
        ...
        tmp_data[3] = 0x00;
        tmp_data[2] = 0x00;
        tmp_data[1] = HX83112B_NF_Data_PWD1;
        tmp_data[0] = HX83112B_NF_Data_PWD0;
        hx83112b_nf_flash_write_burst_length(tmp_addr, tmp_data, 4);

        hx83112b_nf_register_read(tmp_addr, 4, tmp_data, false);
        if ((tmp_data[0] == HX83112B_NF_Data_PWD0 && tmp_data[1] == HX83112B_NF_Data_PWD1) ||
            (tmp_data[0] == HX83112B_NF_Data_PWD1 && tmp_data[1] == HX83112B_NF_Data_PWD0)) {
            break;
        }
    ...
    while (retry < 200) {
        if (tmp_data[0] == HX83112B_NF_Data_PWD1 && tmp_data[1] == HX83112B_NF_Data_PWD0) {
            break;
        }
...
//
    ...
    for (i = 0; i < chip_info->hw_res->RX_NUM; i++) {
        printk("TX%2d", i + 1);
        for (j = 0; j < chip_info->hw_res->TX_NUM; j++) {
            //if ((j == HX83112B_NF_SKIPRXNUM) && (i >= SKIPTXNUM_START) && (i <= SKIPTXNUM_END)) {
               // continue;
            //} else {
...
void hx83112b_nf_switch_data_type(uint8_t checktype)
{
    uint8_t datatype = 0x00;

    switch (checktype) {
        case HIMAX_INSPECTION_SORTING:
            datatype = HX83112B_NF_DATA_SORTING;
        break;
        case HIMAX_INSPECTION_OPEN:
            datatype = HX83112B_NF_DATA_OPEN;
            break;
        case HIMAX_INSPECTION_MICRO_OPEN:
            datatype = HX83112B_NF_DATA_MICRO_OPEN;
            break;
        case HIMAX_INSPECTION_SHORT:
            datatype = HX83112B_NF_DATA_SHORT;
            break;
        case HIMAX_INSPECTION_RAWDATA:
            datatype = HX83112B_NF_DATA_RAWDATA;
            break;
        case HIMAX_INSPECTION_NOISE:
            datatype = HX83112B_NF_DATA_NOISE;
            break;
        case HIMAX_INSPECTION_BACK_NORMAL:
            datatype = HX83112B_NF_DATA_BACK_NORMAL;
            break;
        case HIMAX_INSPECTION_LPWUG_RAWDATA:
            datatype = HX83112B_NF_DATA_LPWUG_RAWDATA;
            break;
        case HIMAX_INSPECTION_LPWUG_NOISE:
            datatype = HX83112B_NF_DATA_LPWUG_NOISE;
            break;
        case HIMAX_INSPECTION_DOZE_RAWDATA:
            datatype = HX83112B_NF_DATA_DOZE_RAWDATA;
            break;
        case HIMAX_INSPECTION_DOZE_NOISE:
            datatype = HX83112B_NF_DATA_DOZE_NOISE;
            break;
        case HIMAX_INSPECTION_LPWUG_IDLE_RAWDATA:
            datatype = HX83112B_NF_DATA_LPWUG_IDLE_RAWDATA;
            break;
...
int hx83112b_nf_switch_mode(int mode)
{
    uint8_t tmp_addr[4];
    uint8_t tmp_data[4];
    TPD_INFO("%s: Entering\n",__func__);
    ...
    case HIMAX_INSPECTION_SORTING:
        tmp_data[3] = 0x00;
        tmp_data[2] = 0x00;
        tmp_data[1] = HX83112B_NF_PWD_SORTING_START;
        tmp_data[0] = HX83112B_NF_PWD_SORTING_START;
        break;
    case HIMAX_INSPECTION_OPEN:
        tmp_data[3] = 0x00;
        tmp_data[2] = 0x00;
        tmp_data[1] = HX83112B_NF_PWD_OPEN_START;
        tmp_data[0] = HX83112B_NF_PWD_OPEN_START;
        break;
    case HIMAX_INSPECTION_MICRO_OPEN:
        tmp_data[3] = 0x00;
        tmp_data[2] = 0x00;
    ...
    case HIMAX_INSPECTION_SHORT:
        tmp_data[3] = 0x00;
        tmp_data[2] = 0x00;
        tmp_data[1] = HX83112B_NF_PWD_SHORT_START;
        tmp_data[0] = HX83112B_NF_PWD_SHORT_START;
        break;
    case HIMAX_INSPECTION_RAWDATA:
        tmp_data[3] = 0x00;
        tmp_data[2] = 0x00;
        tmp_data[1] = HX83112B_NF_PWD_RAWDATA_START;
        tmp_data[0] = HX83112B_NF_PWD_RAWDATA_START;
        break;
    case HIMAX_INSPECTION_NOISE:
        tmp_data[3] = 0x00;
        tmp_data[2] = 0x00;
        tmp_data[1] = HX83112B_NF_PWD_NOISE_START;
        tmp_data[0] = HX83112B_NF_PWD_NOISE_START;
        break;
    case HIMAX_INSPECTION_LPWUG_RAWDATA:
    case HIMAX_INSPECTION_LPWUG_NOISE:
        tmp_data[3] = 0x00;
        tmp_data[2] = 0x00;
        tmp_data[1] = HX83112B_NF_PWD_LPWUG_START;
        tmp_data[0] = HX83112B_NF_PWD_LPWUG_START;
        break;
    case HIMAX_INSPECTION_DOZE_RAWDATA:
    case HIMAX_INSPECTION_DOZE_NOISE:
        tmp_data[3] = 0x00;
        tmp_data[2] = 0x00;
        tmp_data[1] = HX83112B_NF_PWD_DOZE_START;
        tmp_data[0] = HX83112B_NF_PWD_DOZE_START;
        break;
    case HIMAX_INSPECTION_LPWUG_IDLE_RAWDATA:
    case HIMAX_INSPECTION_LPWUG_IDLE_NOISE:
        tmp_data[3] = 0x00;
        tmp_data[2] = 0x00;
        tmp_data[1] = HX83112B_NF_PWD_LPWUG_IDLE_START;
        tmp_data[0] = HX83112B_NF_PWD_LPWUG_IDLE_START;
        break;
    }
    hx83112b_nf_flash_write_burst_length(tmp_addr, tmp_data, 4);
...
void hx83112b_nf_set_N_frame(uint16_t Nframe, uint8_t checktype)
{
    uint8_t tmp_addr[4];
    uint8_t tmp_data[4];
    ...
    case HIMAX_INSPECTION_LPWUG_RAWDATA:
    case HIMAX_INSPECTION_LPWUG_NOISE:
        tmp_data[0] = HX83112B_NF_BS_LPWUG;
        break;
    case HIMAX_INSPECTION_DOZE_RAWDATA:
    case HIMAX_INSPECTION_DOZE_NOISE:
        tmp_data[0] = HX83112B_NF_BS_DOZE;
        break;
    case HIMAX_INSPECTION_LPWUG_IDLE_RAWDATA:
    case HIMAX_INSPECTION_LPWUG_IDLE_NOISE:
        tmp_data[0] = HX83112B_NF_BS_LPWUG_dile;
        break;
    case HIMAX_INSPECTION_RAWDATA:
    case HIMAX_INSPECTION_NOISE:
        tmp_data[0] = HX83112B_NF_BS_RAWDATANOISE;
        break;
    default:
        tmp_data[0] = HX83112B_NF_BS_OPENSHORT;
        break;
    }
    hx83112b_nf_flash_write_burst_length(tmp_addr, tmp_data, 4);
}
...
uint32_t hx83112b_nf_check_mode(uint8_t checktype)
{
    uint8_t tmp_addr[4];
    uint8_t tmp_data[4];
    uint8_t wait_pwd[2];
    ...
    wait_pwd[0] = HX83112B_NF_PWD_NONE;
    wait_pwd[1] = HX83112B_NF_PWD_NONE;

    switch (checktype) {
    case HIMAX_INSPECTION_SORTING:
        wait_pwd[0] = HX83112B_NF_PWD_SORTING_END;
        wait_pwd[1] = HX83112B_NF_PWD_SORTING_END;
        break;
    case HIMAX_INSPECTION_OPEN:
        wait_pwd[0] = HX83112B_NF_PWD_OPEN_END;
        wait_pwd[1] = HX83112B_NF_PWD_OPEN_END;
        break;
    case HIMAX_INSPECTION_MICRO_OPEN:
        wait_pwd[0] = HX83112B_NF_PWD_OPEN_END;
        wait_pwd[1] = HX83112B_NF_PWD_OPEN_END;
        break;
    case HIMAX_INSPECTION_SHORT:
        wait_pwd[0] = HX83112B_NF_PWD_SHORT_END;
        wait_pwd[1] = HX83112B_NF_PWD_SHORT_END;
        break;
    case HIMAX_INSPECTION_RAWDATA:
        wait_pwd[0] = HX83112B_NF_PWD_RAWDATA_END;
        wait_pwd[1] = HX83112B_NF_PWD_RAWDATA_END;
        break;
    case HIMAX_INSPECTION_NOISE:
        wait_pwd[0] = HX83112B_NF_PWD_NOISE_END;
        wait_pwd[1] = HX83112B_NF_PWD_NOISE_END;
        break;
    case HIMAX_INSPECTION_LPWUG_RAWDATA:
    case HIMAX_INSPECTION_LPWUG_NOISE:
        wait_pwd[0] = HX83112B_NF_PWD_LPWUG_END;
        wait_pwd[1] = HX83112B_NF_PWD_LPWUG_END;
        break;
    case HIMAX_INSPECTION_DOZE_RAWDATA:
    case HIMAX_INSPECTION_DOZE_NOISE:
        wait_pwd[0] = HX83112B_NF_PWD_DOZE_END;
        wait_pwd[1] = HX83112B_NF_PWD_DOZE_END;
        break;
    case HIMAX_INSPECTION_LPWUG_IDLE_RAWDATA:
    case HIMAX_INSPECTION_LPWUG_IDLE_NOISE:
        wait_pwd[0] = HX83112B_NF_PWD_LPWUG_IDLE_END;
...
void hx83112b_nf_get_noise_base(void)
{
    uint8_t tmp_addr[4];
    uint8_t tmp_data[4];
...
uint32_t hx83112b_nf_wait_sorting_mode(uint8_t checktype)
{
    uint8_t tmp_addr[4];
    uint8_t tmp_data[4];
    uint8_t wait_pwd[2];
...
int hx83112b_nf_check_notch(int index)
{
    /*if ((index >= HX83112B_NF_SKIP_NOTCH_START) && (index <= HX83112B_NF_SKIP_NOTCH_END))
        return 1;
    else if((index >= HX83112B_NF_SKIP_DUMMY_START) && (index <= HX83112B_NF_SKIP_DUMMY_END))
        return 1;
    else
        return 0;*/
 //   index = ((chip_info->hw_res->RX_NUM * chip_info->hw_res->TX_NUM - i)- chip_info->hw_res->RX_NUM * j) - 1;
    int tx = hx83112b_nf_chip_info->hw_res->TX_NUM;
    int rx = hx83112b_nf_chip_info->hw_res->RX_NUM;
...
static void hx83112b_nf_store_to_file(int fd, char* format, ...)
{
    va_list args;
    char buf[64] = {0};
...
int hx83112b_nf_mpTestFunc(int fd, struct chip_data_hx83112b_nf *chip_info, uint8_t checktype, uint32_t datalen)
{
    uint8_t tmp_addr[4] = {0};
    uint8_t tmp_data[4] = {0};
    ...
    if (hx83112b_nf_check_mode(checktype)) {
        ...
        hx83112b_nf_reload_disable(1);
        #endif

        hx83112b_nf_switch_mode(checktype);
    ...
    hx83112b_nf_switch_data_type(checktype);
    ...
    /* back to normal */
    hx83112b_nf_switch_data_type(HIMAX_INSPECTION_BACK_NORMAL);
    ...
    switch (checktype) {
        case HIMAX_INSPECTION_SORTING:
            ...
            for (i = 0; i < (chip_info->hw_res->TX_NUM * chip_info->hw_res->RX_NUM); i++) {
                if (i % chip_info->hw_res->TX_NUM == 0) {
                    hx83112b_nf_store_to_file(fd, "\n");
                }
                if (hx83112b_nf_check_notch(i)) {
                    hx83112b_nf_store_to_file(fd, "%d, ", RAW[i]);
                    continue;
                }
                if (RAW[i] < HX83112B_NF_SORTINGMIN) {
                    hx83112b_nf_store_to_file(fd, "==>%d, ", RAW[i]);
                    is_test_failed = true;
                } else {
                    hx83112b_nf_store_to_file(fd, "%d, ", RAW[i]);
        ...
        case HIMAX_INSPECTION_OPEN:
            ...
            for (i = 0; i < (chip_info->hw_res->TX_NUM * chip_info->hw_res->RX_NUM); i++) {
                ...
                if (RAW[i] > HX83112B_NF_OPENMAX || RAW[i] < HX83112B_NF_OPENMIN) {
                    hx83112b_nf_store_to_file(fd, "==>%d, ", RAW[i]);
                    is_test_failed = true;
                } else {
                    hx83112b_nf_store_to_file(fd, "%d, ", RAW[i]);
        ...
        case HIMAX_INSPECTION_MICRO_OPEN:
            ...
            for (i = 0; i < (chip_info->hw_res->TX_NUM * chip_info->hw_res->RX_NUM); i++) {
                ...
                if (RAW[i] > HX83112B_NF_M_OPENMAX || RAW[i] < HX83112B_NF_M_OPENMIN) {
                    hx83112b_nf_store_to_file(fd, "==>%d, ", RAW[i]);
                    is_test_failed = true;
                } else {
                    hx83112b_nf_store_to_file(fd, "%d, ", RAW[i]);
        ...
        case HIMAX_INSPECTION_SHORT:
            ...
            for (i = 0; i < (chip_info->hw_res->TX_NUM * chip_info->hw_res->RX_NUM); i++) {
                ...
                if (RAW[i] > HX83112B_NF_SHORTMAX || RAW[i] < HX83112B_NF_SHORTMIN) {
                    hx83112b_nf_store_to_file(fd, "==>%d, ", RAW[i]);
                    is_test_failed = true;
                } else {
                    hx83112b_nf_store_to_file(fd, "%d, ", RAW[i]);
        ...
        case HIMAX_INSPECTION_RAWDATA:
            ...
            for (i = 0; i < (chip_info->hw_res->TX_NUM * chip_info->hw_res->RX_NUM); i++) {
                ...
                if (RAW[i] > HX83112B_NF_RAWMAX || RAW[i] < HX83112B_NF_RAWMIN) {
                    hx83112b_nf_store_to_file(fd, "==>%d, ", RAW[i]);
                    is_test_failed = true;
                } else {
                    hx83112b_nf_store_to_file(fd, "%d, ", RAW[i]);
        ...
        case HIMAX_INSPECTION_NOISE:
            ...
            hx83112b_nf_get_noise_base();
            weight = hx83112b_nf_get_noise_weight();
        ...
        case HIMAX_INSPECTION_LPWUG_RAWDATA:
            ...
            for (i = 0; i < (chip_info->hw_res->TX_NUM * chip_info->hw_res->RX_NUM); i++) {
                ...
                if (RAW[i] > HX83112B_NF_LPWUG_RAWDATA_MAX || RAW[i] < HX83112B_NF_LPWUG_RAWDATA_MIN) {
                    hx83112b_nf_store_to_file(fd, "==>%d, ", RAW[i]);
                    is_test_failed = true;
                } else {
                    hx83112b_nf_store_to_file(fd, "%d, ", RAW[i]);
        ...
        case HIMAX_INSPECTION_DOZE_RAWDATA:
            ...
            for (i = 0; i < (chip_info->hw_res->TX_NUM * chip_info->hw_res->RX_NUM); i++) {
                ...
                if (RAW[i] > HX83112B_NF_DOZE_RAWDATA_MAX || RAW[i] < HX83112B_NF_DOZE_RAWDATA_MIN) {
                    hx83112b_nf_store_to_file(fd, "==>%d, ", RAW[i]);
                    is_test_failed = true;
                } else {
                    hx83112b_nf_store_to_file(fd, "%d, ", RAW[i]);
        ...
        case HIMAX_INSPECTION_DOZE_NOISE:
            ...
            for (i = 0; i < (chip_info->hw_res->TX_NUM * chip_info->hw_res->RX_NUM); i++) {
                ...
                if (RAW[i] > HX83112B_NF_DOZE_NOISE_MAX || RAW[i] < HX83112B_NF_DOZE_NOISE_MIN) {
                    hx83112b_nf_store_to_file(fd, "==>%d, ", RAW[i]);
                    is_test_failed = true;
                } else {
                    hx83112b_nf_store_to_file(fd, "%d, ", RAW[i]);
        ...
        case HIMAX_INSPECTION_LPWUG_IDLE_RAWDATA:
            ...
            for (i = 0; i < (chip_info->hw_res->TX_NUM * chip_info->hw_res->RX_NUM); i++) {
                ...
                if (RAW[i] > HX83112B_NF_LPWUG_IDLE_RAWDATA_MAX || RAW[i] < HX83112B_NF_LPWUG_IDLE_RAWDATA_MIN) {
                    hx83112b_nf_store_to_file(fd, "==>%d, ", RAW[i]);
                    is_test_failed = true;
                } else {
                    hx83112b_nf_store_to_file(fd, "%d, ", RAW[i]);
        ...
        case HIMAX_INSPECTION_LPWUG_IDLE_NOISE:
            ...
            for (i = 0; i < (chip_info->hw_res->TX_NUM * chip_info->hw_res->RX_NUM); i++) {
                ...
                if (RAW[i] > HX83112B_NF_LPWUG_IDLE_NOISE_MAX || RAW[i] < HX83112B_NF_LPWUG_IDLE_NOISE_MIN) {
                    hx83112b_nf_store_to_file(fd, "==>%d, ", RAW[i]);
                    is_test_failed = true;
                } else {
                    hx83112b_nf_store_to_file(fd, "%d, ", RAW[i]);
...
static void hx83112b_nf_black_screen_test(void *chip_data, char *message)
{
    int error = 0;
    int error_num = 0;
    int retry_cnt = 3;
...
#ifndef HX_ZERO_FLASH
    hx83112b_nf_sense_off();
    //hx83112b_nf_set_N_frame(1, HIMAX_INSPECTION_NOISE);

    hx83112b_nf_reload_disable(0);

    hx83112b_nf_sense_on(0);
#endif
    /*hx83112b_nf_mcu_0f_operation_dirly();
    //msleep(5);
    hx83112b_nf_core_fp.fp_reload_disable(0);
    //msleep(5);
    hx83112b_nf_sense_on(0x00);
    hx83112b_nf_read_OPPO_FW_ver();*/
}

int hx83112b_nf_chip_self_test(struct seq_file *s, struct chip_data_hx83112b_nf *chip_info, struct syna_testdata *syna_testdata)
{
    int error = 0;
    int error_num = 0;
    char buf[128] = {0};
    ...
    //hx83112b_nf_set_SMWP_enable(ts->SMWP_enable,suspended);
...
void hx83112b_nf_init_psl(void) //power saving level
{
    uint8_t tmp_addr[4];
    uint8_t tmp_data[4];
...
void hx83112b_nf_chip_erase(void)
{
    uint8_t tmp_addr[4];
    uint8_t tmp_data[4];
...
void hx83112b_nf_flash_programming(uint8_t *FW_content, int FW_Size)
{
...
int hx83112b_nf_fts_ctpm_fw_upgrade_with_sys_fs_64k(unsigned char *fw, int len, bool change_iref) //Alice - Un
{
...
//#endif
    ...
    hx83112b_nf_enter_safe_mode();
    hx83112b_nf_chip_erase();
    hx83112b_nf_flash_programming(fw, FW_SIZE_64k);
    ...
    if(hx83112b_nf_hw_check_CRC(tmp_data, FW_SIZE_64k) == 0) {
        burnFW_success = 1;
    } else {
        burnFW_success = 0;
    }
...
static size_t hx83112b_nf_proc_register_read(struct file *file, char *buf, size_t len, loff_t *pos)
{
    size_t ret = 0;
    uint16_t loop_i;
#ifdef CONFIG_TOUCHPANEL_MTK_PLATFORM
    int max_bus_size = HX83112B_NF_MAX_RECVS_SZ;
    uint8_t data[HX83112B_NF_MAX_RECVS_SZ];
#else
    ...
    if (!HX83112B_NF_HX_PROC_SEND_FLAG) {
        ...
        HX83112B_NF_HX_PROC_SEND_FLAG=0;
    }
    return ret;
}

static size_t hx83112b_nf_proc_register_write(struct file *file, const char *buff, size_t len, loff_t *pos)
{
    char buf[81] = {0};
    char buf_tmp[16];
    uint8_t length = 0;
    ...
    if ((buf[0] == 'r' || buf[0] == 'w') && buf[1] == ':' && buf[2] == 'x') {
        ...
        if (buf[0] == 'r') {
            if (buf[3] == 'F' && buf[4] == 'E' && length == 4) {
                length = length - base;
                hx83112b_nf_cfg_flag = true;
                memcpy(buf_tmp, data_str + base +1, length);
            } else {
                hx83112b_nf_cfg_flag = false;
                memcpy(buf_tmp, data_str + 1, length);
            }

            hx83112b_nf_byte_length = length/2;
            if (!kstrtoul(buf_tmp, 16, &result)) {
                for (loop_i = 0 ; loop_i < hx83112b_nf_byte_length ; loop_i++) {
                    hx83112b_nf_register_command[loop_i] = (uint8_t)(result >> loop_i*8);
                }
            }
        } else if (buf[0] == 'w') {
            if (buf[3] == 'F' && buf[4] == 'E') {
            ...
            if(count < 3) {
                ...
                hx83112b_nf_register_write(hx83112b_nf_register_command, hx83112b_nf_byte_length, w_data, hx83112b_nf_cfg_flag);
            } else {
                ...
                hx83112b_nf_byte_length = count - 1;
                hx83112b_nf_register_write(hx83112b_nf_register_command, hx83112b_nf_byte_length, &w_data[0], hx83112b_nf_cfg_flag);
            }
        } else {
            return len;
        }
...
void hx83112b_nf_return_event_stack(void)
{
    int retry = 20;
    uint8_t tmp_addr[4];
    uint8_t tmp_data[4];
...
int hx83112b_nf_write_read_reg(uint8_t *tmp_addr,uint8_t *tmp_data,uint8_t hb,uint8_t lb)
{
    int cnt = 0;
...
void hx83112b_nf_get_DSRAM_data(uint8_t *info_data, uint8_t x_num, uint8_t y_num)
{
    int i = 0;
    //int cnt = 0;
...
void hx83112b_nf_ts_diag_func(struct chip_data_hx83112b_nf *chip_info, int32_t *mutual_data)
{
    int i = 0;
    int j = 0;
    ...
    hx83112b_nf_burst_enable(1);
    hx83112b_nf_get_DSRAM_data(info_data, chip_info->hw_res->RX_NUM, chip_info->hw_res->TX_NUM);
...
void hx83112b_nf_diag_parse_raw_data(struct hx83112b_nf_report_data *hx83112b_nf_touch_data,int mul_num, int self_num,uint8_t diag_cmd, int32_t *mutual_data, int32_t *self_data)
{
    int RawDataLen_word;
    int index = 0;
    int temp1, temp2,i;

    if (hx83112b_nf_touch_data->hx_rawdata_buf[0] == 0x3A
            && hx83112b_nf_touch_data->hx_rawdata_buf[1] == 0xA3
            && hx83112b_nf_touch_data->hx_rawdata_buf[2] > 0
        ...
        for (i = 0; i < RawDataLen_word; i++) {
            ...
            if (temp1 < mul_num) {
                //mutual
                mutual_data[index + i] = ((int8_t)hx83112b_nf_touch_data->hx_rawdata_buf[i*2 + 4 + 1])*256 + hx83112b_nf_touch_data->hx_rawdata_buf[i*2 + 4];    //4: RawData Header, 1:HSB
            } else {
                //self
                temp1 = i + index;
                ...
                self_data[i+index-mul_num] = (((int8_t)hx83112b_nf_touch_data->hx_rawdata_buf[i*2 + 4 + 1]) << 8) | hx83112b_nf_touch_data->hx_rawdata_buf[i*2 + 4];    //4: RawData Header
                //self_data[i+index-mul_num+1] = hx83112b_nf_touch_data->hx_rawdata_buf[i*2 + 4 + 1];
            }
        }
    }
...
bool hx83112b_nf_diag_check_sum( struct hx83112b_nf_report_data *hx83112b_nf_touch_data ) //return checksum value
{
    uint16_t check_sum_cal = 0;
    int i;

    //Check 128th byte CRC
    for (i = 0, check_sum_cal = 0; i < (hx83112b_nf_touch_data->touch_all_size - hx83112b_nf_touch_data->touch_info_size); i=i+2) {
...
static size_t hx83112b_nf_proc_diag_write(struct file *file, const char *buff, size_t len, loff_t *pos)
{
    char messages[80] = {0};
   ...
   struct chip_data_hx83112b_nf *chip_info = (struct chip_data_hx83112b_nf *)ts->chip_data;
    ...
    if (messages[1] == 0x0A) {
        hx83112b_nf_diag_command = messages[0] - '0';
    } else {
        hx83112b_nf_diag_command = (messages[0] - '0')*10 + (messages[1] - '0');
    }
...
static size_t hx83112b_nf_proc_diag_read(struct file *file, char *buff, size_t len, loff_t *pos)
{
    size_t ret = 0;
    char *temp_buf;
    uint16_t mutual_num;
    ...
    if (!HX83112B_NF_HX_PROC_SEND_FLAG) {
        ...
        dsram_type = hx83112b_nf_diag_command / 10;
        data_type = hx83112b_nf_diag_command % 10;
      ...
      // start to show out the raw data in adb shell
        if ((data_type >= 1 && data_type <= 7)) {
            if (dsram_type > 0)
                hx83112b_nf_ts_diag_func(chip_info, hx83112b_nf_touch_data->diag_mutual);

            for(j=0; j < chip_info->hw_res->RX_NUM ; j++) {
                    for(i=0; i < chip_info->hw_res->TX_NUM; i++) {
                        k = ((mutual_num - j)- chip_info->hw_res->RX_NUM *i) - 1;
                        ret += snprintf(temp_buf + ret, len - ret, "%6d", hx83112b_nf_touch_data->diag_mutual[k]);
                    }
                    ret += snprintf(temp_buf + ret, len - ret, " %6d\n", hx83112b_nf_diag_self[j]);
            }

            ret += snprintf(temp_buf + ret, len - ret, "\n");
            for (i=0; i < chip_info->hw_res->TX_NUM; i++) {
                    ret += snprintf(temp_buf + ret, len - ret, "%6d", hx83112b_nf_diag_self[i]);
            }
        }
        ...
        //if ((hx83112b_nf_diag_command >= 1 && hx83112b_nf_diag_command <= 7) || dsram_type > 0)
            {
            /* print Mutual/Slef Maximum and Minimum */
            //himax_get_mutual_edge();
            for (i = 0; i < (chip_info->hw_res->TX_NUM * chip_info->hw_res->RX_NUM); i++) {
                if (hx83112b_nf_touch_data->diag_mutual[i] > hx83112b_nf_max_mutual) {
                    hx83112b_nf_max_mutual = hx83112b_nf_touch_data->diag_mutual[i];
                }
                if (hx83112b_nf_touch_data->diag_mutual[i] < hx83112b_nf_min_mutual) {
                    hx83112b_nf_min_mutual = hx83112b_nf_touch_data->diag_mutual[i];
                }
            }

            //himax_get_self_edge();
            for (i = 0; i < (chip_info->hw_res->TX_NUM + chip_info->hw_res->RX_NUM); i++) {
                if (hx83112b_nf_diag_self[i] > hx83112b_nf_max_self) {
                    hx83112b_nf_max_self = hx83112b_nf_diag_self[i];
                }
                if (hx83112b_nf_diag_self[i] < hx83112b_nf_min_self) {
                    hx83112b_nf_min_self = hx83112b_nf_diag_self[i];
                }
            }

            ret += snprintf(temp_buf + ret, len - ret, "Mutual Max:%3d, Min:%3d\n", hx83112b_nf_max_mutual, hx83112b_nf_min_mutual);
            ret += snprintf(temp_buf + ret, len - ret, "Self Max:%3d, Min:%3d\n", hx83112b_nf_max_self, hx83112b_nf_min_self);

            /* recovery status after print*/
            hx83112b_nf_max_mutual = 0;
            hx83112b_nf_min_mutual = 0xFFFF;
            hx83112b_nf_max_self = 0;
...
uint8_t hx83112b_nf_read_DD_status(struct chip_data_hx83112b_nf *chip_info, uint8_t *hx83112b_nf_cmd_set, uint8_t *tmp_data)
{
    int cnt = 0;
    uint8_t req_size = hx83112b_nf_cmd_set[0];
    ...
    hx83112b_nf_register_read(tmp_addr, req_size, tmp_data, false);
    return NO_ERR;
}

static size_t hx83112b_nf_proc_DD_debug_read(struct file *file, char *buf, size_t len, loff_t *pos)
{
    int ret = 0;
    uint8_t tmp_data[64];
    uint8_t loop_i = 0;
    ...
    if (!HX83112B_NF_HX_PROC_SEND_FLAG) {
        ...
        if (hx83112b_nf_mutual_set_flag == 1) {
            if (hx83112b_nf_read_DD_status(chip_info, hx83112b_nf_cmd_set, tmp_data) == NO_ERR) {
                for (loop_i = 0; loop_i < hx83112b_nf_cmd_set[0]; loop_i++) {
                    if ((loop_i % 8) == 0) {
...
static size_t hx83112b_nf_proc_DD_debug_write(struct file *file, const char *buff, size_t len, loff_t *pos)
{
    uint8_t i = 0;
    uint8_t cnt = 2;
    unsigned long result = 0;
...
int hx83112b_nf_read_FW_status(struct chip_data_hx83112b_nf *chip_info, uint8_t *state_addr, uint8_t *tmp_addr)
{
    uint8_t req_size = 0;
    ...
    if(state_addr[0]==0x01)
        {
            ...
            req_size = 0x04;
            hx83112b_nf_sense_off();
            hx83112b_nf_register_read(status_addr, req_size, tmp_addr, false);
            hx83112b_nf_sense_on(1);
        }
    else if(state_addr[0]==0x02)
        {
            ...
            req_size = 0x30;
            hx83112b_nf_register_read(cmd_addr, req_size, tmp_addr, false);
        }

    return NO_ERR;
}

static size_t hx83112b_nf_proc_FW_debug_read(struct file *file, char *buf,
                                        size_t len, loff_t *pos)
{
    ...
    if (!HX83112B_NF_HX_PROC_SEND_FLAG) {
        ...
        hx83112b_nf_cmd_set[0] = 0x01;
        if (hx83112b_nf_read_FW_status(chip_info, hx83112b_nf_cmd_set, tmp_data) == NO_ERR) {
            ret += snprintf(temp_buf + ret, len - ret, "0x%02X%02X%02X%02X :\t",hx83112b_nf_cmd_set[5],hx83112b_nf_cmd_set[4],hx83112b_nf_cmd_set[3],hx83112b_nf_cmd_set[2]);
            for (loop_i = 0; loop_i < hx83112b_nf_cmd_set[1]; loop_i++) {
                ret += snprintf(temp_buf + ret, len - ret, "%5d\t", tmp_data[loop_i]);
            }
            ret += snprintf(temp_buf + ret, len - ret, "\n");
        }
        hx83112b_nf_cmd_set[0] = 0x02;
        if (hx83112b_nf_read_FW_status(chip_info, hx83112b_nf_cmd_set, tmp_data) == NO_ERR) {
            for (loop_i = 0; loop_i < hx83112b_nf_cmd_set[1]; loop_i = loop_i + 2) {
                if ((loop_i % 16) == 0)
                    ret += snprintf(temp_buf + ret, len - ret, "0x%02X%02X%02X%02X :\t",
                            hx83112b_nf_cmd_set[5],hx83112b_nf_cmd_set[4],hx83112b_nf_cmd_set[3]+(((hx83112b_nf_cmd_set[2]+ loop_i)>>8)&0xFF), (hx83112b_nf_cmd_set[2] + loop_i)&0xFF);
...
int hx83112b_nf_ic_reset(struct chip_data_hx83112b_nf *chip_info, uint8_t loadconfig,uint8_t int_off)
{
    int ret = 0;
...
static size_t hx83112b_nf_proc_reset_write(struct file *file, const char *buff,
                                 size_t len, loff_t *pos)
{
    char buf_tmp[12];
    struct touchpanel_data *ts = PDE_DATA(file_inode(file));
    ...
    if (buf_tmp[0] == '1')
        hx83112b_nf_ic_reset(chip_info, false,false);
    else if (buf_tmp[0] == '2')
        hx83112b_nf_ic_reset(chip_info,false,true);
    else if (buf_tmp[0] == '3')
        hx83112b_nf_ic_reset(chip_info, true,false);
    else if (buf_tmp[0] == '4')
        hx83112b_nf_ic_reset(chip_info ,true,true);
    else if ((buf_tmp[0] == 'z')&&(buf_tmp[1] == 'r'))
        {
            hx83112b_nf_mcu_0f_operation_check(0);
            hx83112b_nf_mcu_0f_operation_check(1);
        }
...
static size_t hx83112b_nf_proc_sense_on_off_write(struct file *file, const char *buff,
                                        size_t len, loff_t *pos)
{
    char buf[80] = {0};
    //struct touchpanel_data *ts = PDE_DATA(file_inode(file));
...
static int hx83112b_nf_get_touch_points(void *chip_data, struct point_info *points, int max_num)
{
    ...
    int ret = 0;
    int check_sum_cal;
    int ts_status = HX83112B_NF_REPORT_COORD;
    int hx_point_num;
    ...
    if (hx83112b_nf_diag_command)
        ret = hx83112b_nf_read_event_stack(buf, 128);
    else
    ...
    if (LEVEL_DEBUG == tp_debug) {
        hx83112b_nf_log_touch_data(buf, hx83112b_nf_touch_data);
    }

    check_sum_cal = hx83112b_nf_checksum_cal(chip_info, buf, ts_status);//????checksum
    ...
    hx_state_info_pos = hx83112b_nf_touch_data->touch_info_size - 3;
    if(ts_status == HX83112B_NF_REPORT_COORD) {
        memcpy(hx83112b_nf_touch_data->hx_coord_buf,&buf[0],hx83112b_nf_touch_data->touch_info_size);
        if(buf[hx_state_info_pos] != 0xFF && buf[hx_state_info_pos + 1] != 0xFF) {
            memcpy(hx83112b_nf_touch_data->hx_state_info,&buf[hx_state_info_pos],2);
        } else {
            memset(hx83112b_nf_touch_data->hx_state_info, 0x00, sizeof(hx83112b_nf_touch_data->hx_state_info));
        }
    }
    if (hx83112b_nf_diag_command) {
        mutual_num = chip_info->hw_res->TX_NUM * chip_info->hw_res->RX_NUM;
        ...
        hx83112b_nf_diag_parse_raw_data(hx83112b_nf_touch_data, mutual_num, self_num, hx83112b_nf_diag_command, hx83112b_nf_touch_data->diag_mutual, hx83112b_nf_diag_self);
    }

    if (hx83112b_nf_touch_data->hx_coord_buf[HX83112B_NF_TOUCH_INFO_POINT_CNT] == 0xff)//HX83112B_NF_TOUCH_INFO_POINT_CNT buf???????
        hx_point_num = 0;
    else
        hx_point_num= hx83112b_nf_touch_data->hx_coord_buf[HX83112B_NF_TOUCH_INFO_POINT_CNT] & 0x0f;


    for (i = 0; i < 10; i++) {
        x = hx83112b_nf_touch_data->hx_coord_buf[i*4] << 8 | hx83112b_nf_touch_data->hx_coord_buf[i*4 + 1];
        y = (hx83112b_nf_touch_data->hx_coord_buf[i*4 + 2] << 8 | hx83112b_nf_touch_data->hx_coord_buf[i*4 + 3]);
        z = hx83112b_nf_touch_data->hx_coord_buf[i + 40];
        if(x >= 0 && x <= hx83112b_nf_pri_ts->resolution_info.max_x && y >= 0 && y <= hx83112b_nf_pri_ts->resolution_info.max_y) {
            points[i].x = x;
            points[i].y = y;
...
static int hx83112b_nf_ftm_process(void *chip_data)
{
    hx83112b_nf_resetgpio_set(hx83112b_nf_chip_info->hw_res, false); // reset gpio
    return 0;
}

static int hx83112b_nf_get_vendor(void *chip_data, struct panel_info *panel_data)
{
    struct chip_data_hx83112b_nf *chip_info = (struct chip_data_hx83112b_nf *)chip_data;
...
/**
 * hx83112b_nf_get_fw_id -   get device fw id.
 * @chip_info: struct include i2c resource.
 * Return fw version result.
 */
static uint32_t hx83112b_nf_get_fw_id(struct chip_data_hx83112b_nf *chip_info)
{
...
static fw_check_state hx83112b_nf_hw_fw_check(void *chip_data, struct resolution_info *resolution_info, struct panel_info *panel_data)
{
    ...
    panel_data->TP_FW = hx83112b_nf_get_fw_id(chip_info);
    if (panel_data->manufacture_info.version)
        sprintf(panel_data->manufacture_info.version, "0x%x-%d", panel_data->TP_FW, hx83112b_nf_lcd_vendor);

    return FW_NORMAL;
}

static u8 hx83112b_nf_trigger_reason(void *chip_data, int gesture_enable, int is_suspended)
{
...
/*
static int hx83112b_nf_reset_for_prepare(void *chip_data)
{
    int ret = -1;
...
{
    //hx83112b_nf_reset_for_prepare(chip_data);
    #ifdef HX_ZERO_FLASH
 ...
 */
static int hx83112b_nf_reset(void *chip_data)
{
    struct chip_data_hx83112b_nf *chip_info = (struct chip_data_hx83112b_nf *)chip_data;
    int ret = 0;
    int load_fw_times = 10;
...
void hx83112b_nf_ultra_enter(void)
{
    //uint8_t tmp_addr[4];
    uint8_t tmp_data[4];
    int rtimes = 0;
...
/*
static int hx83112b_nf_enable_edge_limit(struct chip_data_hx83112b_nf *chip_info, bool enable)
{
    int ret = 0;
    return ret;
}
...
static int hx83112b_nf_enable_charge_mode(struct chip_data_hx83112b_nf *chip_info, bool enable)
{
    int ret = 0;
    uint8_t tmp_addr[4];
    uint8_t tmp_data[4];
...
//on = 1:on   0:off
static int hx83112b_nf_jitter_switch(struct chip_data_hx83112b_nf *chip_info, bool on)
{
    uint8_t tmp_addr[4];
    uint8_t tmp_data[4];
    int rtimes = 0;
...
static int hx83112b_nf_enable_headset_mode(struct chip_data_hx83112b_nf *chip_info, bool enable)
{
    uint8_t tmp_addr[4];
    uint8_t tmp_data[4];
    int rtimes = 0;
...
static int hx83112b_nf_rotative_switch(struct chip_data_hx83112b_nf *chip_info, int mode)
{
    uint8_t tmp_addr[4];
    uint8_t tmp_data[4];
    int rtimes = 0;
...
void hx83112b_nf_set_SMWP_enable(uint8_t SMWP_enable, bool suspended)
{
    uint8_t tmp_addr[4];
    uint8_t tmp_data[4];
    uint8_t back_data[4];
...
static int hx83112b_nf_get_gesture_info(void *chip_data, struct gesture_info * gesture)
{
    int i = 0;
    int gesture_sign = 0;
    struct chip_data_hx83112b_nf *chip_info = (struct chip_data_hx83112b_nf *)chip_data;
    ...
    int check_sum_cal;
    int ts_status = HX83112B_NF_REPORT_SMWP_EVENT;
    ...
    if (buf[HX83112B_NF_GEST_PTLG_ID_LEN] != HX83112B_NF_GEST_PTLG_HDR_ID1 ||
        buf[HX83112B_NF_GEST_PTLG_ID_LEN+1] != HX83112B_NF_GEST_PTLG_HDR_ID2) {
        goto RET_OUT;
    }

    if (buf[HX83112B_NF_GEST_PTLG_ID_LEN] == HX83112B_NF_GEST_PTLG_HDR_ID1 &&
        buf[HX83112B_NF_GEST_PTLG_ID_LEN+1] == HX83112B_NF_GEST_PTLG_HDR_ID2) {
        gest_len = buf[HX83112B_NF_GEST_PTLG_ID_LEN+2];

        //TPD_DEBUG("gest_len = %d ",gest_len);

        i = 0;
        hx83112b_nf_gest_pt_cnt = 0;
        //TPD_DEBUG("gest doornidate start  %s\n",__func__);
        while (i < (gest_len + 1)/2) {
            if (i == 6) {
                hx83112b_nf_gest_pt_x[hx83112b_nf_gest_pt_cnt] = buf[HX83112B_NF_GEST_PTLG_ID_LEN+4+i*2];
            } else {
                hx83112b_nf_gest_pt_x[hx83112b_nf_gest_pt_cnt] = buf[HX83112B_NF_GEST_PTLG_ID_LEN+4+i*2] * hx83112b_nf_pri_ts->resolution_info.max_x / 255;
            }
            hx83112b_nf_gest_pt_y[hx83112b_nf_gest_pt_cnt] =  buf[HX83112B_NF_GEST_PTLG_ID_LEN+4+i*2+1] * hx83112b_nf_pri_ts->resolution_info.max_y / 255;
            i++;
            //TPD_DEBUG("hx83112b_nf_gest_pt_x[%d]=%d \n",hx83112b_nf_gest_pt_cnt,hx83112b_nf_gest_pt_x[hx83112b_nf_gest_pt_cnt]);
            //TPD_DEBUG("hx83112b_nf_gest_pt_y[%d]=%d \n",hx83112b_nf_gest_pt_cnt,hx83112b_nf_gest_pt_y[hx83112b_nf_gest_pt_cnt]);
            hx83112b_nf_gest_pt_cnt +=1;
        }
        if (hx83112b_nf_gest_pt_cnt) {
             gesture->gesture_type = gesture_sign;//id
             gesture->Point_start.x= hx83112b_nf_gest_pt_x[0];//start x
             gesture->Point_start.y= hx83112b_nf_gest_pt_y[0];//start y
             gesture->Point_end.x  = hx83112b_nf_gest_pt_x[1];//end x
             gesture->Point_end.y  = hx83112b_nf_gest_pt_y[1];//end y
             gesture->Point_1st.x  = hx83112b_nf_gest_pt_x[2];//??1
             gesture->Point_1st.y  = hx83112b_nf_gest_pt_y[2];
             gesture->Point_2nd.x  = hx83112b_nf_gest_pt_x[3];//??2
             gesture->Point_2nd.y  = hx83112b_nf_gest_pt_y[3];
             gesture->Point_3rd.x  = hx83112b_nf_gest_pt_x[4];//??3
             gesture->Point_3rd.y   = hx83112b_nf_gest_pt_y[4];
             gesture->Point_4th.x   = hx83112b_nf_gest_pt_x[5];//??4
             gesture->Point_4th.y   = hx83112b_nf_gest_pt_y[5];
             gesture->clockwise     = hx83112b_nf_gest_pt_x[6]; //??? 1, ??? 0
             //TPD_DEBUG("gesture->gesture_type = %d \n", gesture->gesture_type);
             /*for (i = 0; i < 6; i++)
                TPD_DEBUG("%d [ %d  %d ]\n", i, hx83112b_nf_gest_pt_x[i], hx83112b_nf_gest_pt_y[i]);*/
        }
    }
...
static int hx83112b_nf_power_control(void *chip_data, bool enable)
{
    ...
    if (true == enable) {
        ...
        if (ret)
            return -1;
        ret = hx83112b_nf_resetgpio_set(chip_info->hw_res, true);
        if (ret)
            return -1;
    } else {
        ret = hx83112b_nf_resetgpio_set(chip_info->hw_res, false);
        if (ret)
            return -1;
...
static int hx83112b_nf_int_pin_test(struct seq_file *s, void *chip_data, struct syna_testdata *syna_testdata)
{
...
static void hx83112b_nf_auto_test(struct seq_file *s, void *chip_data, struct syna_testdata *syna_testdata)
{
    int error_count = 0;
    int ret = 0;
    struct chip_data_hx83112b_nf *chip_info = (struct chip_data_hx83112b_nf *)chip_data;
    ...
    error_count += hx83112b_nf_int_pin_test(s, chip_info, syna_testdata);
    error_count += hx83112b_nf_chip_self_test(s, chip_info, syna_testdata);
...
static void hx83112b_nf_read_debug_data(struct seq_file *s, void *chip_data, int HX83112B_NF_DEBUG_DATA_TYPE)
{
    ...
    seq_printf(s, "\n");
    for (i=0; i < chip_info->hw_res->TX_NUM; i++) {
        seq_printf(s, "%6d", hx83112b_nf_diag_self[i]);
    }
    //Clear DSRAM flag
    hx83112b_nf_diag_register_set(0x00);
    HX83112B_NF_DSRAM_Flag = false;
    hx83112b_nf_return_event_stack();
...
static void hx83112b_nf_baseline_read(struct seq_file *s, void *chip_data)
{
    hx83112b_nf_read_debug_data(s, chip_data, DEBUG_DATA_BASELINE);
    hx83112b_nf_read_debug_data(s, chip_data, DEBUG_DATA_RAW);
    return;
}

static void hx83112b_nf_delta_read(struct seq_file *s, void *chip_data)
{
    hx83112b_nf_read_debug_data(s, chip_data, DEBUG_DATA_DELTA);
    return;
}

static void hx83112b_nf_main_register_read(struct seq_file *s, void *chip_data)
{
    return;
}

//Reserved node
static void hx83112b_nf_reserve_read(struct seq_file *s, void *chip_data)
{
    return;
}

static fw_update_state hx83112b_nf_fw_update(void *chip_data, const struct firmware *fw, bool force)
{
    ...
    if (hx83112b_nf_pri_ts->fw_update_app_support && force == 1) {
        ...
        if(fw_name_fae == NULL) {
            ...
            if (p_node != NULL) {
                copy_len = p_node - hx83112b_nf_pri_ts->panel_data.fw_name;
                memcpy(fw_name_fae, hx83112b_nf_pri_ts->panel_data.fw_name, copy_len);
    ...
    if (hx83112b_nf_chip_info->g_fw_entry == NULL){
        if (hx83112b_nf_pri_ts->fw_update_app_support) {
            if(fw_name_fae == NULL) {
                ...
                err = request_firmware(&hx83112b_nf_chip_info->g_fw_entry,  fw_name_fae, hx83112b_nf_pri_ts->dev);
            }
        } else {
            err = request_firmware(&hx83112b_nf_chip_info->g_fw_entry,  hx83112b_nf_pri_ts->panel_data.fw_name, hx83112b_nf_pri_ts->dev);
        }
...
static int hx83112b_nf_get_usb_state(void)
{
...
static int hx83112b_nf_reset_gpio_control(void *chip_data, bool enable)
{
    struct chip_data_hx83112b_nf *chip_info = (struct chip_data_hx83112b_nf *)chip_data;
...
static void hx83112b_nf_set_touch_direction(void *chip_data, uint8_t dir)
{
        struct chip_data_hx83112b_nf *chip_info = (struct chip_data_hx83112b_nf *)chip_data;
...
static uint8_t hx83112b_nf_get_touch_direction(void *chip_data)
{
        struct chip_data_hx83112b_nf *chip_info = (struct chip_data_hx83112b_nf *)chip_data;
...
int hx83112b_nf_freq_point = 0;
void hx83112b_nf_freq_hop_trigger(void *chip_data)
{
    uint8_t tmp_addr[4];
    uint8_t tmp_data[4];
    int rtimes = 0;
...
static struct oppo_touchpanel_operations hx83112b_nf_ops = {
    .ftm_process      = hx83112b_nf_ftm_process,
    .get_vendor       = hx83112b_nf_get_vendor,
    .get_chip_info    = hx83112b_nf_get_chip_info,
    .reset            = hx83112b_nf_reset,
    .power_control    = hx83112b_nf_power_control,
    .fw_check         = hx83112b_nf_hw_fw_check,
    .fw_update        = hx83112b_nf_fw_update,
    .trigger_reason   = hx83112b_nf_trigger_reason,
    .get_touch_points = hx83112b_nf_get_touch_points,
    .get_gesture_info = hx83112b_nf_get_gesture_info,
    .mode_switch      = hx83112b_nf_mode_switch,
    .exit_esd_mode    = hx83112b_nf_exit_esd_mode,
    //.resume_prepare   = hx83112b_resume_prepare,
    .get_usb_state    = hx83112b_nf_get_usb_state,
    .black_screen_test = hx83112b_nf_black_screen_test,
    .reset_gpio_control = hx83112b_nf_reset_gpio_control,
    .set_touch_direction    = hx83112b_nf_set_touch_direction,
    .get_touch_direction    = hx83112b_nf_get_touch_direction,
    .freq_hop_trigger = hx83112b_nf_freq_hop_trigger,
};

static struct himax_proc_operations hx83112b_nf_proc_ops = {
    .auto_test     = hx83112b_nf_auto_test,
    .himax_proc_register_write =  hx83112b_nf_proc_register_write,
    .himax_proc_register_read =  hx83112b_nf_proc_register_read,
    .himax_proc_diag_write =  hx83112b_nf_proc_diag_write,
    .himax_proc_diag_read =  hx83112b_nf_proc_diag_read,
    .himax_proc_DD_debug_read =  hx83112b_nf_proc_DD_debug_read,
    .himax_proc_DD_debug_write =  hx83112b_nf_proc_DD_debug_write,
    .himax_proc_FW_debug_read =  hx83112b_nf_proc_FW_debug_read,
    .himax_proc_reset_write =  hx83112b_nf_proc_reset_write,
    .himax_proc_sense_on_off_write =  hx83112b_nf_proc_sense_on_off_write,
};

static struct debug_info_proc_operations hx83112b_nf_debug_info_proc_ops = {
    .limit_read    = himax_limit_read,
    .delta_read    = hx83112b_nf_delta_read,
    .baseline_read = hx83112b_nf_baseline_read,
    .main_register_read = hx83112b_nf_main_register_read,
    .reserve_read = hx83112b_nf_reserve_read,
};

static int hx83112b_nf_tp_probe(struct spi_device *spi)
{
    struct chip_data_hx83112b_nf *chip_info = NULL;
    ...
    hx83112b_nf_chip_info = chip_info;

    /* allocate himax report data */
    hx83112b_nf_touch_data = kzalloc(sizeof(struct hx83112b_nf_report_data),GFP_KERNEL);
    if(hx83112b_nf_touch_data == NULL)
    {
    ...
    chip_info->syna_ops = &hx83112b_nf_proc_ops;
    ts->debug_info_ops = &hx83112b_nf_debug_info_proc_ops;
    ts->s_client = spi;
    ...
    memcpy(&chip_info->hx_spi_mcc, &hx83112b_nf_spi_ctrdata, sizeof(struct mtk_chip_config));
    ts->s_client->controller_data = (void *)&chip_info->hx_spi_mcc;
#else
    /* old usage of MTK spi API */
    memcpy(&chip_info->hx_spi_mcc, &hx83112b_nf_spi_ctrdata, sizeof(struct mt_chip_conf));
    ts->s_client->controller_data = (void *)&chip_info->hx_spi_mcc;
    ...
    ts->ts_ops = &hx83112b_nf_ops;

    hx83112b_nf_pri_ts = ts;
    ...
    INIT_DELAYED_WORK(&chip_info->work_0f_update, hx83112b_nf_mcu_0f_operation);
    //queue_delayed_work(chip_info->himax_0f_update_wq, &chip_info->work_0f_update, msecs_to_jiffies(2000));
#else
    hx83112b_nf_read_FW_ver();
    hx83112b_nf_calculateChecksum(false);
#endif

    hx83112b_nf_power_on_init();

    //touch data init
    ret = hx83112b_nf_report_data_init(ts->max_num,  ts->hw_res.TX_NUM, ts->hw_res.RX_NUM);
    if (ret ) {
        goto err_register_driver;
    }
...
static int hx83112b_nf_tp_remove(struct spi_device *spi)
{
...
static int hx83112b_nf_i2c_suspend(struct device *dev)
{
...
static int hx83112b_nf_i2c_resume(struct device *dev)
{
    struct touchpanel_data *ts = dev_get_drvdata(dev);
...
static const struct spi_device_id hx83112b_nf_id[] = {
...
static struct of_device_id hx83112b_nf_match_table[] = {
#ifdef CONFIG_TOUCHPANEL_MULTI_NOFLASH
...
static const struct dev_pm_ops hx83112b_nf_pm_ops = {
#ifdef CONFIG_FB
    .suspend = hx83112b_nf_i2c_suspend,
    .resume = hx83112b_nf_i2c_resume,
#endif
};


static struct spi_driver hx83112b_nf_common_driver = {
    .probe      = hx83112b_nf_tp_probe,
    .remove     = hx83112b_nf_tp_remove,
    .id_table   = hx83112b_nf_id,
    .driver = {
        .name = TPD_DEVICE,
        .owner = THIS_MODULE,
...
static int __init hx83112b_nf_driver_init(void)
{
    int status = 0;
...
static void __exit hx83112b_nf_driver_exit(void)
{
    spi_unregister_driver(&hx83112b_nf_common_driver);
    return;
}

module_init(hx83112b_nf_driver_init);
module_exit(hx83112b_nf_driver_exit);
