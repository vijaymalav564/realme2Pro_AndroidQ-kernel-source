...
//static kal_bool bat_update_thread_timeout = KAL_FALSE;//sjc20151224
//struct timespec battery_duration_time[DURATION_NUM]={0};//sec
//kal_uint32 wake_up_smooth_time = 0; //sec
//kal_uint32 battery_tracking_time;
...
void mt_battery_update_time(struct timespec * pre_time, BATTERY_TIME_ENUM duration_type)
{
	...
	battery_log(BAT_LOG_CRTI,"[Battery] mt_battery_update_duration_time , last_time=%d current_time=%d duration=%d\n",
...
kal_uint32 mt_battery_get_duration_time(BATTERY_TIME_ENUM duration_type)
{
...
{
	return oppo_chg_get_chg_temperature()/10;
}
...
{
	if ((oppo_chg_get_batt_full() == KAL_TRUE) && (oppo_chg_get_rechging_status() == KAL_FALSE))
		return KAL_TRUE;
	else
		return KAL_FALSE;
}
...
#endif
//void mt_battery_update_status(void)//sjc
//{
//#if defined(CONFIG_POWER_EXT)
...
CHARGER_TYPE mt_charger_type_detection(void)
{
	...
	chg_debug("mt_charger_type_detection------1--------CHR_Type_num = %d\r\n",CHR_Type_num);

	mutex_lock(&charger_type_mutex);
	if(pmic_chrdet_status() == false)
	{
		CHR_Type_num = CHARGER_UNKNOWN;
	}
	else
	{
		if(oppo_chg_get_chg_type() == POWER_SUPPLY_TYPE_UNKNOWN)
		{
	...
	chg_debug("mt_charger_type_detection------2--------CHR_Type_num = %d\r\n",CHR_Type_num);
...
{
	if(oppo_gauge_check_chip_is_null() /*|| oppo_vooc_check_chip_is_null()*/ || oppo_chg_check_chip_is_null())
	{
...
static long adc_cali_unlocked_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	case Get_META_BAT_VOL:
		...
		adc_out_data[0] = oppo_chg_get_batt_volt();
		ret = copy_to_user(user_data_addr, adc_out_data, 8);
	...
	case Get_META_BAT_SOC:
		...
		user_data_addr = (int *)arg;
    		ret = copy_from_user(adc_in_data, user_data_addr, 8);
		adc_out_data[0] = oppo_chg_get_ui_soc();
		ret = copy_to_user(user_data_addr, adc_out_data, 8); 
	...
	case Get_FakeOff_Param: /* For Factory Mode*/
		...
		fakeoff_out_data[3] = oppo_vooc_get_fastchg_started();
		ret = copy_to_user(user_data_addr, fakeoff_out_data, 16);
		chg_debug("ioctl : Get_FakeOff_Param:ui_soc:%d, g_NotifyFlag:%d,chr_det:%d,fast_chg = %d\n",fakeoff_out_data[0],fakeoff_out_data[1],fakeoff_out_data[2],fakeoff_out_data[3]);

		break; 
			
	case Get_Notify_Param: /* For Fakeoff Mode*/
		chg_debug("ioctl : Get_Notify_Param\n"); 
		break; 
		
	case Turn_Off_Charging: /* For Turnoffcharging Mode*/
		chg_debug("ioctl : Turn_Off_Charging\n"); 
		break;

	case K_AT_CHG_CHGR_IN:
		chg_debug("ioctl : K_AT_CHG_CHGR_IN\n"); 
		break;
		
	case K_AT_CHG_CHGR_OFF:
		chg_debug("ioctl : K_AT_CHG_CHGR_OFF\n");
		break;
		
	case K_AT_CHG_ON:
		chg_debug("ioctl : K_AT_CHG_ON\n");
		break;
		
	case K_AT_CHG_OFF:
		chg_debug("ioctl : K_AT_CHG_OFF\n");
		break;
		
	case K_AT_CHG_INFO:
		chg_debug("ioctl : K_AT_CHG_INFO\n");
		break;
		
	case SET_SPI_CS_LOW:
		chg_debug("ioctl : SET_SPI_CS_LOW\n");
		break;
	default:
...
#endif
#if 0
int bat_update_thread(void *x)//sjc20151224
{
	/* Run on a process content */
	while (1) {
		mutex_lock(&bat_update_mutex);
		//sjc mt_battery_update_status();
...
void bat_update_thread_wakeup(void)//sjc20151224
{
...
static int battery_probe(struct platform_device *dev)    
{
	...
	chg_debug("battery_probe----------------soi\n");
	//struct oppo_chg_chip	*chip;
	struct class_device *class_dev = NULL;	
	int ret = 0;
	int ret_device_file=0;
#if 0	
	chip = devm_kzalloc(&dev->dev,
...
static int __init battery_init(void)
{
	...
	chg_debug("battery_init-----------4----------CONFIG_OF\r\n");
    ...
    if (ret)
        chg_debug("failed to register PM notifier %d\n", ret);
